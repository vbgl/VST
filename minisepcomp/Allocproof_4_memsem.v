...
Lemma exec_moves:
  ...
  wt_regset env rs ->
  exists ls',
    corestep_star LTL_core_sem tge (Block s f sp (expand_moves mv bb) ls retty) m
...
+ exploit IHmv; eauto. eapply subst_loc_undef_satisf; eauto.
  intros [ls' [A B]]. exists ls'; split; auto. eapply corestep_star_trans; eauto.
  apply corestep_star_one. econstructor. simpl. eauto. auto.
  (* reg->stack *)
+ exploit IHmv; eauto. eapply subst_loc_undef_satisf; eauto.
  intros [ls' [A B]]. exists ls'; split; auto. eapply corestep_star_trans; eauto.
  apply corestep_star_one. econstructor. simpl. eauto. 
  (* stack->reg *)
+ simpl in Heqb. exploit IHmv; eauto. eapply subst_loc_undef_satisf; eauto.
  intros [ls' [A B]]. exists ls'; split; auto. eapply corestep_star_trans; eauto.
  apply corestep_star_one. econstructor. auto. 
  (* stack->stack *)
...
(*CompComp adaptation: state refactoring, added retty, 
 ported star -> corestep_star*)
Inductive match_stackframes: list RTL_memsem.stackframe -> list LTL_memsem.stackframe -> signature -> Prop :=
  ...
  | match_stackframes_cons:
      forall res f sp pc rs s tf bb ls ts sg an e env 
        ...
        (STEPS: forall v ls1 m retty,
           ...
           exists ls2,
           corestep_star LTL_core_sem tge (Block ts tf sp bb ls1 retty) m
...
(*CompComp adaptation: state refactoring, added retty*)
Inductive match_states: RTL_memsem.state -> mem -> LTL_memsem.state -> mem -> Prop :=
  | match_states_intro:
      ...
      match_states (RTL_memsem.State s f sp pc rs) m
  ...
  | match_states_call:
      ...
      match_states (RTL_memsem.Callstate s f args) m
  ...
  | match_states_return:
      ...
      match_states (RTL_memsem.Returnstate s res) m
...
Lemma step_simulation:
  forall S1 m1 S2 m2 , RTL_memsem.step ge S1 m1 S2 m2 -> wt_state S1 m1 ->
  forall S1' m1', match_states S1 m1 S1' m1' ->
  exists S2' m2', corestep_plus LTL_core_sem tge S1' m1' S2' m2' /\ match_states S2 m2 S2' m2'.
Proof.
  induction 1; intros WT S1' m1' MS; inv MS; try UseShape.

(* nop *)
- exploit exec_moves; eauto. intros [ls1 [X Y]].
  eexists; econstructor; split.
  eapply corestep_one_star_one_plus.
    econstructor; eauto.
    eexact X.
    econstructor; eauto.
...
- generalize (wt_exec_Iop _ _ _ _ _ _ _ _ _ _ _ WTI H0 WTRS). intros WTRS'.
  ...
  eexists; econstructor; split.
  eapply corestep_one_star_one_star_one_plus.
    econstructor; eauto.
    eexact A1. 
...
- generalize (wt_exec_Iload _ _ _ _ _ _ _ _ _ _ _ WTI H1 WTRS). intros WTRS'.
  ...
  * eapply corestep_star_trans. eexact A3. 
...
- exploit exec_moves; eauto. intros [ls1 [X Y]].
  ...
  eapply corestep_one_star_one_plus.
    ...
    eapply corestep_star_one. econstructor. instantiate (1 := a'). rewrite <- F.
      apply eval_addressing_preserved. exact symbols_preserved. eauto. eauto.
    constructor.
  exploit satisf_successors; eauto. simpl; eauto.
...
- exploit Mem.storev_int64_split; eauto.
  ...
  eapply corestep_one_star_one_star_one_plus.
    ...
    eapply corestep_star_trans. eexact U. 
      eapply corestep_star_one. econstructor. eexact F2''. eexact STORE2'. eauto.
    constructor.
  exploit satisf_successors; eauto. simpl; eauto.
  eapply can_undef_satisf. eauto.
  eapply add_equation_satisf. eapply add_equations_satisf; eauto.
...
- set (sg := RTL_memsem.funsig fd) in *.
  ...
  intros [ls2 [A2 B2]].
  exists ls2; split.
  eapply corestep_star_trans. eexact A2. apply corestep_star_one. constructor. 
...
+ (* without an argument *)
  ...
  constructor.
  (*CompComp adaptation: new retty clause:*)
  * intros; subst. rewrite <- H11. eauto.
