...
(*CompComp adaptation: state refactoring*)
Inductive match_states: Csharpminor_memsem.state -> mem -> Cminor_memsem.state -> mem -> Prop :=
  | match_state:
      ...
      match_states (Csharpminor_memsem.State fn s k e le) m
  ...
  | match_state_seq:
      ...
      match_states (Csharpminor_memsem.State fn (Csharpminor_memsem.Sseq s1 s2) k e le) m
  ...
  | match_callstate:
      ...
      match_states (Csharpminor_memsem.Callstate fd args k) m
  ...
  | match_returnstate:
      ...
      match_states (Csharpminor_memsem.Returnstate v k) m
...
Lemma switch_match_states:
  ...
  /\ match_states (Csharpminor_memsem.State fn (seq_of_lbl_stmt ls) k e le) m S tm.
Proof.
  intros. inv TK.
- econstructor; split.
...
Lemma transl_step_correct:
  ...
  forall T1 tm1, match_states S1 m1 T1 tm1 ->
  (exists T2 tm2, corestep_plus CMin_memsem tge T1 tm1 T2 tm2 /\ match_states S2 m2 T2 tm2)
  \/ (measure S2 < measure S1 /\ match_states S2 m2 T1 tm1)%nat.
Proof.
  induction 1; intros T1 tm1 MSTATE; inv MSTATE.
...
Lemma transl_final_states:
  forall S m R tm r,
  match_states S m R tm -> CSharpMin_halted S = Some r ->
...
Lemma transl_atExt_states:
  forall S m R tm f sg args,
  match_states S m R tm -> CSharpMin_at_external S = Some (f, sg, args) ->
...
End TRANSLATION. (*CompCert 2.7.1 has 2233 lines, so if we remove the approx 40 or so lines 
where we comment out CompCert lemmas, we have a overhead of < 100 lines - less that 5%.
This overhead is mostly due to the new lemma transl_afterExt_states, and the longer transl_initial_states*)
