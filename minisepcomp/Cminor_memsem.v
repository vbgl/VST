...
(*COMPCOMP adaptation: removed mem components in all constructors*)
...
Inductive step: state -> mem -> state -> mem -> Prop :=
  ...
  | step_internal_function: forall f vargs k m m' sp e,
      ...
      step (Callstate (Internal f) vargs k) m
        (State f f.(fn_body) k (Vptr sp Int.zero) e) m'
...
Definition CMin_initial_core (ge:genv) (v: val) (args:list val): option state :=
   match v with
        Vptr b i => 
          ...
          then match Genv.find_funct_ptr ge b with
                 ...
                 | Some f => 
                    match f with Internal fi =>
                      if (*CompComp Todo: reactive these conditions later 
                         val_has_type_list_func args (sig_args (funsig f))
                         && vals_defined args
...
Definition CMin_at_external (c: state) : option (external_function * signature * list val) :=
  match c with
  | State f s k sp e => None
  | Callstate fd args k => match fd with
                                  Internal f => None
...
*)
(*CompComp: don't require return value to be a Vint*)
Definition CMin_halted (q : state): option val :=
    match q with 
       Returnstate v Kstop => Some v
     | _ => None
    end.

Definition CMin_after_external (vret: option val) (c: state) : option state :=
  match c with 
    Callstate fd args k => 
         match fd with
            Internal f => None
...
Lemma CMin_corestep_not_at_external:
       forall ge m q m' q', step ge q m q' m' -> CMin_at_external q = None.
...
Lemma CMin_corestep_not_halted : forall ge m q m' q', 
       step ge q m q' m' -> CMin_halted q = None.
...
Definition CMin_core_sem : CoreSemantics genv state mem.
Proof.
...
Lemma cmin_step_mem : forall g c m c' m' (CS: step g c m c' m'), mem_step m m'.
Proof. intros.
...
Program Definition CMin_memsem : MemSem genv state.
Proof.
apply Build_MemSem with (csem := CMin_core_sem).
  apply cmin_step_mem.
Defined.
...
Inductive estep: (block -> Z -> bool) -> state -> mem -> state -> mem -> Prop :=
  ...
  | estep_skip_seq: forall f s k sp e m,
      estep EmptyEffect (State f Sskip (Kseq s k) sp e) m
        (State f s k sp e) m
  | estep_skip_block: forall f k sp e m,
      estep EmptyEffect (State f Sskip (Kblock k) sp e) m
        (State f Sskip k sp e) m
  | estep_skip_call: forall f k sp e m m',
      is_call_cont k ->
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      estep (FreeEffect m 0 (f.(fn_stackspace)) sp)
            (State f Sskip k (Vptr sp Int.zero) e) m
            (Returnstate Vundef k) m'

  | estep_assign: forall f id a k sp e m v,
      eval_expr ge sp e m a v ->
      estep EmptyEffect (State f (Sassign id a) k sp e) m
         (State f Sskip k sp (PTree.set id v e)) m

  | estep_store: forall f chunk addr a k sp e m vaddr v m',
      eval_expr ge sp e m addr vaddr ->
      eval_expr ge sp e m a v ->
      Mem.storev chunk m vaddr v = Some m' ->
      estep (StoreEffect vaddr (encode_val chunk v))
            (State f (Sstore chunk addr a) k sp e) m
        (State f Sskip k sp e) m'

  | estep_call: forall f optid sig a bl k sp e m vf vargs fd,
      eval_expr ge sp e m a vf ->
      eval_exprlist ge sp e m bl vargs ->
      Genv.find_funct ge vf = Some fd ->
      funsig fd = sig ->
      estep EmptyEffect (State f (Scall optid sig a bl) k sp e) m
         (Callstate fd vargs (Kcall optid f sp e k)) m

  | estep_tailcall: forall f sig a bl k sp e m vf vargs fd m',
      ...
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      estep (FreeEffect m 0 (f.(fn_stackspace)) sp) 
            (State f (Stailcall sig a bl) k (Vptr sp Int.zero) e) m
            (Callstate fd vargs (call_cont k)) m'

  | estep_builtin: forall f optid ef bl k sp e m vargs t vres m'
      (*COMPCOMP adaptation: limit to unobervables:*) (NOBS:~ observableEF (*hf*) ef),
      eval_exprlist ge sp e m bl vargs ->
      external_call ef ge vargs m t vres m' ->
      estep (BuiltinEffect ge ef vargs m) (State f (Sbuiltin optid ef bl) k sp e) m
          (State f Sskip k sp (set_optvar optid vres e)) m'

  | estep_seq: forall f s1 s2 k sp e m,
      estep EmptyEffect (State f (Sseq s1 s2) k sp e) m
         (State f s1 (Kseq s2 k) sp e) m

  | estep_ifthenelse: forall f a s1 s2 k sp e m v b,
      eval_expr ge sp e m a v ->
      Val.bool_of_val v b ->
      estep EmptyEffect (State f (Sifthenelse a s1 s2) k sp e) m
         (State f (if b then s1 else s2) k sp e) m

  | estep_loop: forall f s k sp e m,
      estep EmptyEffect (State f (Sloop s) k sp e) m
         (State f s (Kseq (Sloop s) k) sp e) m

  | estep_block: forall f s k sp e m,
      estep EmptyEffect (State f (Sblock s) k sp e) m
         (State f s (Kblock k) sp e) m

  | estep_exit_seq: forall f n s k sp e m,
      estep EmptyEffect (State f (Sexit n) (Kseq s k) sp e) m
         (State f (Sexit n) k sp e) m
  | estep_exit_block_0: forall f k sp e m,
      estep EmptyEffect (State f (Sexit O) (Kblock k) sp e) m
        (State f Sskip k sp e) m
  | estep_exit_block_S: forall f n k sp e m,
      estep EmptyEffect (State f (Sexit (S n)) (Kblock k) sp e) m
         (State f (Sexit n) k sp e) m

  | estep_switch: forall f islong a cases default k sp e m v n,
      eval_expr ge sp e m a v ->
      switch_argument islong v n ->
      estep EmptyEffect (State f (Sswitch islong a cases default) k sp e) m
        (State f (Sexit (switch_target n default cases)) k sp e) m

  | estep_return_0: forall f k sp e m m',
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      estep (FreeEffect m 0 (f.(fn_stackspace)) sp)
            (State f (Sreturn None) k (Vptr sp Int.zero) e) m
            (Returnstate Vundef (call_cont k)) m'
  | estep_return_1: forall f a k sp e m v m',
      eval_expr ge (Vptr sp Int.zero) e m a v ->
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      estep (FreeEffect m 0 (f.(fn_stackspace)) sp) 
            (State f (Sreturn (Some a)) k (Vptr sp Int.zero) e) m
            (Returnstate v (call_cont k)) m'

  | estep_label: forall f lbl s k sp e m,
      estep EmptyEffect (State f (Slabel lbl s) k sp e) m
        (State f s k sp e) m

  | estep_goto: forall f lbl k sp e m s' k',
      find_label lbl f.(fn_body) (call_cont k) = Some(s', k') ->
      estep EmptyEffect (State f (Sgoto lbl) k sp e) m
        (State f s' k' sp e) m

  | estep_internal_function: forall f vargs k m m' sp e,
      Mem.alloc m 0 f.(fn_stackspace) = (m', sp) ->
      set_locals f.(fn_vars) (set_params vargs f.(fn_params)) = e ->
  ...
  | estep_return: forall v optid f sp e k m,
      estep EmptyEffect (Returnstate v (Kcall optid f sp e k)) m
        (State f Sskip k sp (set_optvar optid v e)) m.
...
intros.
  ...
  split; [ econstructor; eauto |]. eapply FreeEffect_free; eassumption. 
  ...
  split; [ econstructor; eauto |]. eapply FreeEffect_free; eassumption.
  split; [ econstructor; eauto |]. eapply FreeEffect_free; eassumption.  
  split; [ econstructor; eauto |]. eapply EmptyEffect_alloc; eassumption.
...
Lemma cmin_eff_perm: forall (E : block -> Z -> bool) ge c m c' m'
...
Program Definition CMin_effsem : 
  @EffectSem genv state.
Proof.
eapply Build_EffectSem with (sem := CMin_memsem (*hf*))
       (effstep:=estep).
...
(*CompComp: not sure we'll need this
