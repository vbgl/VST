...
Definition AccessMap_EI_Property (L1 B2: block -> bool) (j:meminj) (m1 m1' m2 : mem)
           (AM:ZMap.t (Z -> perm_kind -> option permission)):Prop :=
  forall b2, 
    (Mem.valid_block m2 b2 -> forall k ofs2,
       if B2 b2 then PMap.get b2 AM ofs2 k  = PMap.get b2 m2.(Mem.mem_access) ofs2 k else
(*       if L1 b2 then PMap.get b2 AM ofs2 k = PMap.get b2 m1'.(Mem.mem_access) ofs2 k else*)
       match j b2 with 
         None => if L1 b2 then PMap.get b2 AM ofs2 k  = 
                               PMap.get b2 m2.(Mem.mem_access) ofs2 k
                 else PMap.get b2 AM ofs2 k  = PMap.get b2 m1'.(Mem.mem_access) ofs2 k
       | Some (b3,d3) =>
...
Lemma EI_ok: forall (m1 m2 m1':mem)
               ...
               (Inj13': Mem.inject j' m1' m3') L1 L3
         (UnchSrc : Mem.unchanged_on (fun b1 _ => L1 b1 = true /\ j b1 = None) m1 m1')
         (UnchTgt : Mem.unchanged_on (o_o_reach j L3 m1) m3 m3')
         (INC : mini_extern_incr j j' L1 L3)
(*         (GSep : globals_separate g3 j j')*) B1 B3
         (MV : meminj_valid j' L1 m1' L3 m3') m2' B2
         (HB2: forall b2, B2 b2 = true -> B1 b2 = true)
         (HB1: forall b1, B1 b1 = true -> L1 b1 = false)
         (RDOnlyfwd1: RDOnly_fwd m1 m1' B1) (RDOnly_fwd3: RDOnly_fwd m3 m3' B3)

               (NB: m2'.(Mem.nextblock)=m1'.(Mem.nextblock))
               (CONT: Content_EI_Property j m1 m1' m2 (m2'.(Mem.mem_contents)))
               (ACCESS: AccessMap_EI_Property L1 B2  j m1 m1' m2 (m2'.(Mem.mem_access))),
Mem.extends m1' m2' /\
  mem_forward m2 m2' /\
  Mem.inject j' m2' m3' /\
  RDOnly_fwd m2 m2' B2 /\
...
{ split. 
  ...
  + intros. destruct (ACCESS b) as [Val _].
...
{ split. 
  + rewrite NB, <- (Mem.mext_next _ _ Ext12). eapply mem_forward_nextblock. trivial. 
  + intros. destruct H as [Lb Jb]. destruct (ACCESS b) as [Val _].
    specialize (Val H0 k ofs). rewrite Jb, Lb in Val.
    remember (B2 b) as q; destruct q; rewrite (perm_subst _ _ _ _ _ _ _ Val); split; auto.
  + intros. destruct H as [Lb Jb].
    destruct (CONT b) as [ContVal _]. rewrite Jb in ContVal.
    apply ContVal. eapply Mem.perm_valid_block; eassumption. }
...
{ split; intros. 
  ...
  + (*max*)
      ...
      remember (B2 b) as q.
      destruct q.
      - rewrite (perm_subst _ _ _ _ _ _ _ H2) in H0; trivial. 
      - remember (j b) as jb.
        ...
        * remember (L1 b) as w; symmetry in Heqw; destruct w.
          ++ rewrite (perm_subst _ _ _ _ _ _ _ H2) in *; trivial.
          ++ rewrite (perm_subst _ _ _ _ _ _ _ H2) in *.
             eapply Mem.perm_extends. eassumption. apply Fwd1; trivial.
             eapply Mem.valid_block_extends; eassumption. 
      - exfalso. apply (H1 H). }
assert (MMU_LOOR: Mem.unchanged_on (o_o_reach (Mem.flat_inj (Mem.nextblock m1)) L1 m1) m2 m2'). 
{ split.
  + apply MMU_LU.
  + intros. destruct H as [Lb Pb]. 
    apply (valid_split _ _ _ _  (ACCESS b)); clear ACCESS; intros; try contradiction.
    remember (B2 b) as q; destruct q.
    - rewrite (perm_subst b b m2' m2 ofs ofs k); trivial. split; auto.
    - remember (j b) as jb; destruct jb; apply eq_sym in Heqjb.
      * destruct p0.
              destruct (H1 k ofs) as [_ NoMax]; clear H1.
              rewrite (perm_subst b b m2' m2 ofs ofs k). split; auto.
              apply NoMax; clear NoMax. 
              specialize (Pb b 0). rewrite Zminus_0_r, flatinj_I in Pb.
              apply Pb; trivial. rewrite (Mem.mext_next _ _ Ext12). apply H0.
      * rewrite Lb in H1. rewrite (perm_subst b b m2' m2 ofs ofs k); trivial. split; auto.
  + intros. destruct H as [Lb Pb]. destruct (CONT b) as [ValC _].
       ...
       - destruct p.
         eapply (perm_split _ _ _ _ _ _ _ ValC); clear ValC; intros.
             exfalso. specialize (Pb b 0). rewrite Zminus_0_r, flatinj_I in Pb.
             apply Pb; trivial. 
  ...
  + (*mem_inj*)
     ...
     { intros. 
       ...
       + assert (Val1: Mem.valid_block m1 b).
           solve [apply (Mem.valid_block_extends _ _ _ Ext12); apply H0].
         assert (Perm1: Mem.perm m1 b ofs Max Nonempty).
         { apply Fwd1. apply Val1. 
           eapply Mem.perm_max. eapply Mem.perm_implies.
           apply H. constructor. }
         specialize (H1 k ofs).
         remember (B2 b) as q; destruct q; symmetry in Heqq.
         - admit. (*need readonly-property of all blocks in B2*) 
         - remember (j b) as jb.
           destruct jb; apply eq_sym in Heqjb.
           * destruct p0.
           ...
           * remember (L1 b) as w; symmetry in Heqw; destruct w.
             ++ rewrite (perm_subst _ _ _ _ _ _ _ H1) in *; clear H1.
                destruct UnchSrc as [_ UP _].
                eapply (extends_perm _ _ Ext12).
                rewrite (UP _ _ _ p); trivial. split; trivial.
             ++ rewrite (perm_subst _ _ _ _ _ _ _ H1) in *; clear H1; trivial.
    ...
    split. 
         ...
         (*mi_memval *) intros. inv H. rewrite Zplus_0_r. admit. (*Values
            ...
            apply (valid_split _ _ _ _  (ACCESS b2)); clear ACCESS; intros.
               ...
               specialize (ContVal H ofs). specialize (H1 Cur ofs).
                ...
                destruct (ContInval H VB1' ofs) as [Cont _]. 
                  ...
                  apply memval_inject_id_refl.*)
    + intros.
       apply (valid_split _ _ _ _  (ACCESS b)); clear ACCESS; intros VB.
       - assert (Val1: Mem.valid_block m1 b).
           solve [apply (Mem.valid_block_extends _ _ _ Ext12); apply VB].
         intros ACC. specialize (ACC k ofs).
         remember (B2 b) as q; symmetry in Heqq.
         destruct q.
         * rewrite (perm_subst _ _ _ _ _ _ _ ACC) in *; clear ACC.
           destruct (Mem.perm_dec m1' b ofs Max Nonempty). 2: right; trivial.
           left. destruct (Mem.mext_perm_inv _ _ Ext12 _ _ _ _ H). 2: solve [elim H0; apply Fwd1; trivial].
           apply HB2 in Heqq. apply RDOnlyfwd1 in Heqq. red in Heqq.
           admit. (*need readonly property of blocks in B1*)
         * remember (j b) as jb.
           destruct jb; apply eq_sym in Heqjb.
           ++ destruct p0.
              apply (perm_split _ _ _ _ _ _ _ ACC); clear ACC; intros.  
              rewrite (perm_subst _ _ _ _ _ _ _ H1) in H. left; assumption.
              right. intros N. apply H0. apply Fwd1; trivial.
           ++ remember (L1 b) as w; symmetry in Heqw; destruct w.
               -- rewrite (perm_subst _ _ _ _ _ _ _ ACC) in *; clear ACC.
                  destruct UnchSrc as [_ UP _].
                  destruct (Mem.perm_dec m1' b ofs Max Nonempty). 2: right; trivial.
                  left. destruct (Mem.mext_perm_inv _ _ Ext12 _ _ _ _ H). 2: solve [elim H0; apply Fwd1; trivial].
                  apply UP; trivial. split; trivial.
               -- rewrite (perm_subst _ _ _ _ _ _ _ ACC) in H. left; assumption.
       - intros. apply (perm_split _ _ _ _ _ _ _ (H0 k ofs)); clear H0; intros. 
         * rewrite (perm_subst _ _ _ _ _ _ _ H1) in H. left; assumption.
         * right;  assumption. }
{ (* Mem.inject j' m2' m3').*)
  ...
  { assert (MiPerm: forall b1 b2 delta ofs k p,  j' b1 = Some (b2, delta) ->
    ...
    + intros b2 b3; intros.
      ...
      - assert (Val1: Mem.valid_block m1 b2).
        ...
        * rewrite (perm_subst _ _ _ _ _ _ _ (H2 k ofs)) in *. 
          rewrite (perm_subst _ _ _ _ _ _ _ (H2 Max ofs)) in *. clear H2.
          assert (Jb1: j b2 = Some (b3, delta) /\ B3 b3 = true). admit. (*RDO*)
          destruct Jb1 as [Jb1 B3_b3].
          eapply RDOnly_fwd3; trivial. 
          instantiate (1:=55). admit. (*RDONLY*) admit. (*RDONLY*)
          eapply Inj23. eassumption. trivial.
        * remember (j b2) as jb2; symmetry in Heqjb2. destruct jb2.
          Focus 2. destruct (INC2 _ _ _ Heqjb2 H). rewrite H3 in *.
                    rewrite (perm_subst _ _ _ _ _ _ _ (H2 k ofs)) in *; clear H2.
                    eapply Inj13'. eassumption. eassumption.
          destruct p0. specialize (INC1 _ _ _ Heqjb2). rewrite INC1 in H. inv H.
          destruct (Mem.perm_dec m1 b2 ofs Max Nonempty).
          ++ destruct (H2 k ofs) as [X _]. rewrite (perm_subst _ _ _ _ _ _ _ (X p0)) in *; clear X.
             eapply Inj13'. eassumption. eassumption.
          ++ destruct (H2 k ofs) as [_ X]. rewrite (perm_subst _ _ _ _ _ _ _ (X n)) in *; clear X.
             destruct (H2 Max ofs) as [_ X]. rewrite (perm_subst _ _ _ _ _ _ _ (X n)) in *; clear H2 X.
             apply UnchTgt. red.
             red in MV. destruct (Mem.mext_perm_inv _ _ Ext12 _ _ _ _ NP). contradiction. specialize (Mem.
             eapply Inj13'. eassumption. eassumption.
  clear H2.
          eapply UnchTgt.
          ++ red.  rewrite J in H2.
