...
Require Import minisepcomp.Ordered.
Require Import AST.
Require Import Maps.
...
Inductive val_casted: val -> type -> Prop :=
  ...
  | val_casted_float32: forall attr n,
      val_casted (Vsingle n) (Tfloat F32 attr)
  | val_casted_float64: forall attr n,
...
(*  | val_casted_ptr_cptr: forall b ofs id attr,
  ...
  | val_casted_ptr_cptr: forall b ofs t attr,
      val_casted (Vptr b ofs) (Tpointer t attr)
  | val_casted_int_cptr: forall n t attr,
      val_casted (Vint n) (Tpointer t attr)
...
(*    | Vfloat n, Tfloat sz attr =>
    ...
    (*| Vptr b ofs, (*Tcomp_ptr id attr*) Tpointer t attr => true*)
    (*| Vint n, (*Tcomp_ptr id attr*) Tpointer t attr => true*)
...
case_eq (Int.eq_dec (cast_int_int i0 s i) i). intros e _ _.
constructor; auto. intros n _; inversion 1.
destruct f0; try discriminate. constructor.
destruct f0; try discriminate. constructor.
destruct i0; try inversion 1. constructor.
Qed.
...
Qed.
*)
(*CompComp2.7.1: Now takes memory*)
Lemma cast_val_is_casted:
  forall v ty ty' m v', sem_cast v ty ty' m = Some v' -> val_casted v' ty'.
Proof.
  unfold sem_cast; intros. destruct ty'; simpl in *.
(* void *)
+ constructor.
(* int *)
+ destruct i; destruct ty; simpl in H; try discriminate; destruct v; inv H; try solve[ destruct f; discriminate].
- constructor. apply (cast_int_int_idem I8 s).
- constructor. apply (cast_int_int_idem I8 s).
- destruct f; inv H1.
  destruct (cast_float_int s f0); inv H0. constructor. apply (cast_int_int_idem I8 s). 
- destruct f; inv H1.
  destruct (cast_single_int s f0); inv H0. constructor. apply (cast_int_int_idem I8 s). 
- constructor. apply (cast_int_int_idem I16 s).
- constructor. apply (cast_int_int_idem I16 s).
- destruct f; inv H1. destruct (cast_float_int s f0); inv H0.
  constructor. apply (cast_int_int_idem I16 s). 
- destruct f; inv H1. destruct (cast_single_int s f0); inv H0.
  constructor. apply (cast_int_int_idem I16 s).
- constructor. auto.
- constructor.
- constructor. auto. 
- destruct f; inv H1.
  destruct (cast_float_int s f0); inv H0. constructor. auto.
- destruct f; inv H1.
  destruct (cast_single_int s f0); inv H0. constructor. auto.
- constructor. auto.
- constructor.
- constructor; auto.
- constructor.
...
- destruct f; inv H1. constructor.
  destruct (Float.cmp Ceq f0 Float.zero); auto. 
- destruct f; inv H1. constructor. simpl.
...
- destruct f; inv H.
  * destruct v; try discriminate. 
    destruct (cast_single_long s f); try discriminate. inv H1. constructor.
  * destruct v; try discriminate.
    destruct (cast_float_long s f); try discriminate. inv H1. constructor.
- destruct v; inv H. constructor.
- destruct v; inv H. constructor.
- destruct v; inv H. constructor.
(* float *)
+ destruct f; inv H.
- destruct ty; simpl in H1; try discriminate; destruct v; inv H1; try solve [constructor]; try solve [destruct f; discriminate].
  (* unfold cast_float_float, cast_int_float.*)
  * destruct f; try discriminate. inv H0. constructor.
  * destruct f; try discriminate. inv H0. constructor.
- destruct ty; simpl in H1; try discriminate; destruct v; inv H1; try solve [constructor]; try solve [destruct f; discriminate].
  * destruct f; try discriminate. inv H0. constructor.
  * destruct f; try discriminate. inv H0. constructor.
...
+ destruct ty; try discriminate; destruct v; try discriminate.
  destruct (ident_eq i0 i); inv H; constructor.
(* comp_ptr destruct ty; simpl in H; try discriminate; destruct v; inv H; constructor.*)
Qed.
...
Proof.
  intros. inversion H; clear H; subst v ty; simpl in H0; try solve [inv H0; auto].
  destruct sz.
...
Proof.
split.
+ induction v; auto; simpl; destruct t; auto.
+
induction v; simpl; auto; destruct t; auto; solve [inversion 1].
Qed.
...
Lemma vals_lessdef_defined vl1 vl2 : 
  Forall2 Val.lessdef vl1 vl2 -> 
  vals_defined vl1=true -> 
  vals_defined vl2=true.
Proof. 
induction 1; simpl; trivial.
destruct x; simpl in *; intros; try discriminate; inv H; eauto.
Qed.

Lemma val_list_lessdef_hastype vl1 vl2 tys :
  Forall2 Val.lessdef vl1 vl2 ->
  vals_defined vl1=true -> 
  val_has_type_list_func vl1 tys=true ->
  val_has_type_list_func vl2 tys=true.
...
split.
+ rewrite <-val_has_type_funcP. inv H3. rewrite andb_true_iff in H0. destruct H0.
  rewrite <-val_has_type_funcP in H.
  simpl in H2. destruct a; try discriminate; inv H5; trivial.
+ eapply (IHvl1 l'); eauto.
  inv H3. rewrite H0. rewrite andb_true_iff in H0. 
  solve[destruct H0 as [_ ->]; auto].
Qed.
...
Proof.
intros H0 H.
apply val_casted_funcE in H. simpl in H0.
induction H; simpl; trivial. discriminate. 
Qed.
