...
Require Import FSets Coqlib Maps minisepcomp.Ordered minisepcomp.Iteration Errors.
...
Require Import minisepcomp.Unusedglob.
...
Inductive match_stacks (j: meminj) (L L': block -> bool):
        list stackframe -> list stackframe -> block -> block -> Prop :=
  | match_stacks_nil: forall bound tbound
     (*(*COMPCOMP new:*) (Gext: forall b, Ple b (Genv.genv_next ge) -> L b = false)
     (*COMPCOMP new:*) (Gext': forall b', Ple b' (Genv.genv_next tge) -> L' b' = false)*)
     (*COMPCOMP new:*) (Gext: forall b x, Genv.find_def ge b = Some x -> L b = false)
     (*COMPCOMP new:*) (Gext': forall b' x, Genv.find_def tge b' = Some x -> L' b' = false),
      meminj_preserves_globals j ->
      Ple (Genv.genv_next ge) bound -> Ple (Genv.genv_next tge) tbound ->
      match_stacks j L L' nil nil bound tbound
  | match_stacks_cons: forall res f sp pc rs s tsp trs ts bound tbound
         (STACKS: match_stacks j L L' s ts sp tsp)
         (KEPT: forall id, ref_function f id -> kept id)
         (SPINJ: j sp = Some(tsp, 0))
         (REGINJ: regset_inject j rs trs)
         (BELOW: Plt sp bound)
         (TBELOW: Plt tsp tbound)
         (*COMPCOMP new:*)(Lsp: L sp = true)
         (*COMPCOMP new:*)(Ltsp: L' tsp = true),
      match_stacks j L L' (Stackframe res f (Vptr sp Int.zero) pc rs :: s)
                     (Stackframe res f (Vptr tsp Int.zero) pc trs :: ts)
                     bound tbound.

Lemma match_stacks_preserves_globals:
  forall j L L' s ts bound tbound,
  match_stacks j L L' s ts bound tbound ->
  meminj_preserves_globals j.
Proof.
  induction 1; auto.
Qed.
...
Lemma match_stacks_incr_gsep:
  forall j j', inject_incr j j' ->
  forall L L' s ts bound tbound m1, match_stacks j L L' s ts bound tbound ->
  globals_separate m1 tge j j' -> Ple (Genv.genv_next ge) (Mem.nextblock m1) ->
(*  (forall b1 b2 delta,
      j b1 = None -> j' b1 = Some(b2, delta) -> Genv.find_def tge b2 = None) ->*)
  match_stacks j' L L' s ts bound tbound.
Proof.
  induction 2; intros.
- assert (SAME: forall b b' id delta, Genv.find_symbol ge id = Some b ->
                                   j' b = Some(b', delta) -> j b = Some(b', delta)).
  { intros. destruct (j b) as [[b1 delta1] | ] eqn: J.
    exploit H; eauto. congruence.
    destruct (H3 _ _ _ J H6).
    apply Genv.genv_symb_range in H5. unfold Mem.valid_block in H7. xomega. }
  assert (SAME_DEF: forall b b' x delta, Genv.find_def ge b = Some x ->
                                   j' b = Some(b', delta) -> j b = Some(b', delta)).
  { intros. destruct (j b) as [[b1 delta1] | ] eqn: J.
    exploit H; eauto. congruence.
    specialize (H3 _ _ _ J H6).
    apply Genv.genv_defs_range in H5. unfold Mem.valid_block in H3. xomega. }
  assert (SAME': forall b b' delta, Plt b' (Genv.genv_next tge) ->
                                   j' b = Some(b', delta) -> j b = Some (b', delta)).
  { intros. destruct (j b) as [[b1 delta1] | ] eqn: J.
    exploit H; eauto. congruence. specialize (H3 _ _ _ J H6). unfold Mem.valid_block in H3. xomega. }
...
Lemma match_stacks_incr:
  forall j j', inject_incr j j' ->
  forall L L' s ts bound tbound, match_stacks j L L' s ts bound tbound ->
  (forall b1 b2 delta,
...
Lemma match_stacks_bound:
  forall j L L' s ts bound tbound bound' tbound',
  match_stacks j L L' s ts bound tbound ->
...
Lemma match_stacks_sub:
  forall j L L' s ts bound tbound
  (MS: match_stacks j L L' s ts bound tbound)
   K (HK: forall b, L b = true -> K b = true)
  K' (HK': forall b, L' b = true -> K' b = true)
  (Glob: forall b, Plt b (Genv.genv_next ge) -> K b = false)
  (Glob': forall b, Plt b (Genv.genv_next tge) -> K' b = false),
  match_stacks j K K' s ts bound tbound.
Proof.
  induction 1; intros. constructor; trivial.
  destruct H. intros. apply Glob. eapply Genv.genv_defs_range; apply H.
              intros. apply Glob'. eapply Genv.genv_defs_range; apply H2.
  constructor; eauto.
Qed.
Lemma match_stacks_sub':
  forall j L L' s ts bound tbound
  (MS: match_stacks j L L' s ts bound tbound)
   K (HK: forall b, L b = true -> K b = true)
  K' (HK': forall b, L' b = true -> K' b = true)
  (Gext: forall b x, Genv.find_def ge b = Some x -> K b = false)
  (Gext': forall b x, Genv.find_def tge b = Some x -> K' b = false),
  match_stacks j K K' s ts bound tbound.
Proof.
  induction 1; intros. constructor; trivial.
  constructor; eauto.
...
Lemma match_stacks_glob:
  forall j L L' s ts bound tbound
  (MS: match_stacks j L L' s ts bound tbound),
  (*(forall b : positive, Plt b (Genv.genv_next ge) -> L b = false) /\ 
  (forall b : positive, Plt b (Genv.genv_next tge) -> L' b = false).*)
  (forall b x, Genv.find_def ge b = Some x -> L b = false) /\ 
   (forall b' x, Genv.find_def tge b' = Some x -> L' b' = false).
Proof. induction 1; eauto. Qed.

(*Compcomp: expose j, add L and L', and refector RTL states*)
Inductive match_states L L' j :  RTL_memsem.state -> mem -> RTL_memsem.state -> mem -> Prop:=
  | match_states_regular: forall s f sp pc rs m ts tsp trs tm 
         (STACKS: match_stacks j L L' s ts sp tsp)
         (KEPT: forall id, ref_function f id -> kept id)
         (SPINJ: j sp = Some(tsp, 0))
         (REGINJ: regset_inject j rs trs)
      ...
      match_states L L' j (State s f (Vptr sp Int.zero) pc rs) m
                          (State ts f (Vptr tsp Int.zero) pc trs) tm
  | match_states_call: forall s fd args m ts targs tm 
         (STACKS: match_stacks j L L' s ts (Mem.nextblock m) (Mem.nextblock tm))
         (KEPT: forall id, ref_fundef fd id -> kept id)
         (ARGINJ: Val.inject_list j args targs)
         (MEMINJ: Mem.inject j m tm),
      match_states L L' j (Callstate s fd args) m
                          (Callstate ts fd targs) tm
  | match_states_return: forall s res m ts tres tm 
         (STACKS: match_stacks j L L' s ts (Mem.nextblock m) (Mem.nextblock tm))
         (RESINJ: Val.inject j res tres)
         (MEMINJ: Mem.inject j m tm),
      match_states L L' j (Returnstate s res) m
...
Definition match_cores (j:meminj) (L:block -> bool) (st:RTL_memsem.state) (m:mem) (L':block -> bool) (st':RTL_memsem.state) (m':mem) : Prop :=
  match_states L L' j st m st' m' /\ 
 (forall b, L b = true -> Mem.valid_block m b) /\ (forall b, L' b = true -> Mem.valid_block m' b) /\
 (forall b b' d, j b = Some(b',d) -> L b = L' b') /\
 (Ple (Genv.genv_next ge) (Mem.nextblock m)) /\
 (Ple (Genv.genv_next tge) (Mem.nextblock m')).

Lemma match_cores_inject j L1 c1 m1 L2 c2 m2 (MC: match_cores j L1 c1 m1 L2 c2 m2):
      Mem.inject j m1 m2.
Proof. destruct MC as [MC [HL HL']]. inv MC; trivial. Qed.

Lemma match_cores_valid_left j L1 c1 m1 L2 c2 m2 (MC: match_cores j L1 c1 m1 L2 c2 m2)
      b (Hb: L1 b = true): Mem.valid_block m1 b.
Proof. eapply MC; eassumption. Qed.

Lemma match_cores_valid_right j L1 c1 m1 L2 c2 m2 (MC: match_cores j L1 c1 m1 L2 c2 m2)
...
Proof.
  intros. destruct (plt p0 q). left; auto. right; xomega.
Qed.

Lemma core_diagram: forall (st1 : state) (m1 : mem) (st1' : state) (m1' : mem) (U1 : block -> Z -> bool)
  (STEP: effect_semantics.effstep RTL_eff_sem ge U1 st1 m1 st1' m1')
  (st2 : state) (j : meminj) (m2 : mem)
  (L1 L2 : block -> bool) (MC: match_cores j L1 st1 m1 L2 st2 m2),
exists
  (st2' : state) (m2' : mem) (j' : meminj) (L1' L2' : block -> bool),
  ...
  (exists U2 : block -> Z -> bool,
     effect_semantics.effstep RTL_eff_sem tge U2 st2 m2 st2' m2' /\
  ...
  match_cores j' L1' st1' m1' L2' st2' m2'.
Proof.
  specialize mini_intern_incr_same_meminj; intros INC_SAME.
  specialize mini_locally_allocated_same; intros LOCALLOC_SAME.

  induction 1; intros; destruct MC as [MS [HL1 [HL2 [HLL [GV1 GV2]]]]]; subst; inv MS;
  specialize (globals_separate_same_meminj m tge); intros GSEP_SAME.

- (* nop *)
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Inop; eauto.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. intuition. 

- (* op *)
  assert (A: exists tv,
               eval_operation tge (Vptr tsp Int.zero) op trs##args m2 = Some tv
            /\ Val.inject j v tv).
  ...
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Iop; eauto.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. apply set_reg_inject; auto. intuition. 
...
- (* load *)
  ...
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Iload; eauto.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. apply set_reg_inject; auto. intuition.
...
- (* call *)
  ...
  intros (A & B).
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Icall; eauto.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto.
         econstructor; eauto.
         change (Mem.valid_block m sp0). eapply Mem.valid_block_inject_1; eauto.
         change (Mem.valid_block m2 tsp). eapply Mem.valid_block_inject_2; eauto.  
         apply regs_inject; auto.
...
- (* tailcall *)
  ...
  specialize ( Mem.valid_block_free_1 _ _ _ _ _ C); intros VB'.
  eexists; exists tm', j, L1, L2. intuition.
    eapply mini_locally_allocated_parallel_free; eassumption.
  econstructor; split. eapply effexec_Itailcall; eauto.
    intros HH bb ofs EFF. 
...
- (* builtin *)
  ...
  assert (FwdTgt: mem_forward m2 tm') by (eapply external_call_mem_forward; eauto).
  eexists; exists tm', j'.
  exists (fun b => L1 b || freshblock m m' b), (fun b => L2 b || freshblock m2 tm' b).
  intuition.
  * split; trivial; intros.
    destruct (G _ _ _ J J') as [vb vb'].
    assert (Fb: freshblock m m' b1 = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_1; eauto. }
    assert (Fb': freshblock m2 tm' b2 = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_2; eauto. }
    rewrite Fb, Fb'. split; apply orb_true_r.
  * red; intros. destruct (G _ _ _ J J').
    split; trivial. 
    destruct (Plt_Ple_dec b2 (Genv.genv_next tge)); trivial. elim H4. unfold Mem.valid_block. xomega.
  * split; trivial.
  * econstructor; split.
  ...
  * split.
      exploit match_stacks_glob; try eassumption. intros [XX YY].
      eapply match_states_regular with (j := j'); try eassumption.
      apply match_stacks_incr with j; auto.
      eapply match_stacks_sub'; try eassumption.
        intros b Lb; rewrite Lb; trivial.
        intros b Lb; rewrite Lb; trivial.
        intros b x Hb. rewrite (XX _ _ Hb); simpl. apply freshblock_charF. left.
             apply Genv.genv_defs_range in Hb. unfold Mem.valid_block; xomega.
        intros b x Hb. rewrite (YY _ _ Hb); simpl. apply freshblock_charF. 
             apply Genv.genv_defs_range in Hb. unfold Mem.valid_block; xomega. 
      intros. exploit G; eauto. intros [U V].
      assert (Mem.valid_block m sp0) by (eapply Mem.valid_block_inject_1; eauto).
      assert (Mem.valid_block m2 tsp) by (eapply Mem.valid_block_inject_2; eauto).
      unfold Mem.valid_block in *; xomega.
      apply F; eauto.
      apply set_res_inject; auto. apply regset_inject_incr with j; auto.
      rewrite Lsp; trivial.
      rewrite Ltsp; trivial.
    repeat split; intros.
    ++ apply orb_true_iff in H3. destruct H3.
       eapply FwdSrc; eauto.
       unfold freshblock in H3. destruct (valid_block_dec m' b); trivial; discriminate.
    ++ apply orb_true_iff in H3. destruct H3.
       eapply FwdTgt; eauto.
       unfold freshblock in H3. destruct (valid_block_dec tm' b); trivial; discriminate.
    ++ remember (j b) as q; symmetry in Heqq; destruct q.
       -- destruct p0. rewrite (F _ _ _ Heqq) in H3; inv H3. 
          rewrite (HLL _ _ _ Heqq).
          assert (Fb: freshblock m m' b = false). { apply freshblock_charF. left. eapply Mem.valid_block_inject_1; eauto. }
          assert (Fb': freshblock m2 tm' b' = false). { apply freshblock_charF. left. eapply Mem.valid_block_inject_2; eauto. }
          rewrite Fb, Fb'; trivial.
       -- destruct (G _ _ _ Heqq H3).
          assert (Fb: freshblock m m' b = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_1; eauto. }
          assert (Fb': freshblock m2 tm' b' = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_2; eauto. }
          rewrite Fb, Fb'. do 2 rewrite orb_true_r; trivial.
   ++ eapply Ple_trans; eauto. apply forward_nextblock; trivial. 
   ++ eapply Ple_trans; eauto. apply forward_nextblock; trivial. 
  
- (* cond *)
  assert (C: eval_condition cond trs##args m2 = Some b).
  { eapply eval_condition_inject; eauto. apply regs_inject; auto. }
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Icond with (pc' := if b then ifso else ifnot); eauto. 
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. intuition.
...
- (* jumptbl *)
  ...
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_Ijumptable; eauto.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. intuition.
...
- (* return *)
  ...
  eexists; exists tm', j, L1, L2. intuition.
    eapply mini_locally_allocated_parallel_free; eassumption.
  econstructor; split. eapply effexec_Ireturn; eauto.
    intros HH bb ofs EFF.
...
- (* internal function *)
  ...
  assert (TSTK: tstk = Mem.nextblock m2) by (eapply Mem.alloc_result; eauto).
  assert (STACKS': match_stacks j' (fun b => L1 b || eq_block stk b)
                                   (fun b => L2 b || eq_block tstk b) s ts stk tstk).
  { rewrite STK, TSTK.
    exploit match_stacks_glob; try eassumption. intros [XX YY].
    apply match_stacks_incr with j; auto.
      eapply match_stacks_sub'; try eassumption.
      ...
      intros b Hb; rewrite Hb; trivial.
      intros b x Hb; rewrite (XX _ _ Hb); simpl. destruct (eq_block (Mem.nextblock m) b); trivial.
         subst; exfalso. apply Genv.genv_defs_range in Hb. xomega. 
      intros b x Hb; rewrite (YY _ _ Hb); simpl. destruct (eq_block (Mem.nextblock m2) b); trivial.
         subst; exfalso. apply Genv.genv_defs_range in Hb. xomega.
    intros. destruct (eq_block b1 stk).
    subst b1. rewrite F in H1; inv H1. subst b2. split; apply Ple_refl.
    rewrite G in H1 by auto. congruence. }
  eexists; exists tm', j'.
  exists (fun b => L1 b || eq_block stk b). exists (fun b => L2 b || eq_block tstk b).
  intuition.
  * split; trivial; intros. specialize (G b1).
    destruct (eq_block stk b1); subst stk tstk; simpl.
    ++ subst b1. rewrite F in J'; inv J'.
       destruct (eq_block (Mem.nextblock m2) (Mem.nextblock m2)); try congruence; simpl. split; apply orb_true_r.
    ++ rewrite G in J'; congruence. 
  * red; intros. specialize (G b1).
    destruct (eq_block b1 stk); subst; simpl.
            ++ rewrite J' in F; inv F. split; trivial. unfold Mem.valid_block; xomega.
            ++ rewrite G in J'; trivial; congruence. 
  * red. erewrite freshblock_alloc; try eassumption.
         erewrite freshblock_alloc; try eassumption. split; trivial. 
  * econstructor; split. eapply effexec_function_internal; eauto.
      intros HH bb ofs EFF; inv EFF.
  * split.
      eapply match_states_regular with (j := j'); eauto.
      apply init_regs_inject; auto. apply val_inject_list_incr with j; auto.
      destruct (eq_block stk stk). apply orb_true_r. congruence.
      destruct (eq_block tstk tstk). apply orb_true_r. congruence.
    repeat split; intros.
    ...
    + apply orb_true_iff in H0; destruct H0.
      -- eapply Mem.valid_block_alloc; eauto.
      -- destruct (eq_block tstk b); try discriminate. subst b. eapply Mem.valid_new_block; eauto. 
    + specialize (G b).
      destruct (eq_block stk b); subst; simpl. rewrite e in *.
     -- rewrite F in H0; inv H0. destruct (eq_block (Mem.nextblock m2) (Mem.nextblock m2)); try congruence; simpl. do 2 rewrite orb_true_r; trivial.
     -- rewrite G in H0; try congruence.
       destruct (eq_block (Mem.nextblock m2) b'); subst; simpl.
       ** elim (Mem.fresh_block_alloc _ _ _ _ _ C). eapply MEMINJ; eauto.
       ** do 2 rewrite orb_false_r; eauto.
    + eapply Ple_trans; eauto. apply forward_nextblock. eapply alloc_forward; eauto. 
    + eapply Ple_trans; eauto. apply forward_nextblock. eapply alloc_forward; eauto.  

- (* external function - now observable*)
  exploit external_call_inject; eauto.
  eapply match_stacks_preserves_globals; eauto.
  ...
  * red; intros. destruct (G _ _ _ J J'). 
    split; trivial. 
           destruct (Plt_Ple_dec b2 (Genv.genv_next tge)); trivial. elim H1; unfold Mem.valid_block; xomega.
  * split; trivial.
  * econstructor; split. eapply effexec_function_external; eauto.
      intros HH bb ofs EFF. right.
      eapply BuiltinEffects_propagate_injects; eassumption.
  * exploit match_stacks_glob; try eassumption. intros [XX YY].
    split. eapply match_states_return with (j := j'); eauto.
      apply match_stacks_bound with (Mem.nextblock m) (Mem.nextblock m2).
      apply match_stacks_incr with j; auto.
      eapply match_stacks_sub'; try eassumption.
        intros b Hb; rewrite Hb; trivial.
        intros b Hb; rewrite Hb; trivial.
        intros b x Hb. rewrite (XX _ _ Hb); simpl. apply freshblock_charF. left. apply Genv.genv_defs_range in Hb. unfold Mem.valid_block; xomega. 
        intros b x Hb. rewrite (YY _ _ Hb); simpl. apply freshblock_charF. left. apply Genv.genv_defs_range in Hb. unfold Mem.valid_block; xomega. 
      intros. exploit G; eauto. intros [U V].
      unfold Mem.valid_block in *; xomega.
    ...
    ++ apply orb_true_iff in H0. destruct H0.
       eapply FwdTgt; eauto.
       unfold freshblock in H0. destruct (valid_block_dec tm' b); trivial; discriminate.
    ++ remember (j b) as q; symmetry in Heqq; destruct q.
       -- destruct p0. rewrite (F _ _ _ Heqq) in H0; inv H0. 
          rewrite (HLL _ _ _ Heqq).
          assert (Fb: freshblock m m' b = false). { apply freshblock_charF. left. eapply Mem.valid_block_inject_1; eauto. }
          assert (Fb': freshblock m2 tm' b' = false). { apply freshblock_charF. left. eapply Mem.valid_block_inject_2; eauto. }
          rewrite Fb, Fb'; trivial.
       -- destruct (G _ _ _ Heqq H0).
          assert (Fb: freshblock m m' b = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_1; eauto. }
          assert (Fb': freshblock m2 tm' b' = true). { apply freshblockProp_char. split; trivial. eapply Mem.valid_block_inject_2; eauto. }
          rewrite Fb, Fb'. do 2 rewrite orb_true_r; trivial.
    ++ eapply Ple_trans; eauto. apply forward_nextblock; trivial.
...
- (* return *)
  inv STACKS.
  eexists; exists m2, j, L1, L2. intuition.
  econstructor; split. eapply effexec_return.
    intros HH bb ofs EFF; inv EFF.
  split. econstructor; eauto. apply set_reg_inject; auto.
  intuition.
Qed.
...
eapply Mini_simulation_inj.Build_Mini_simulation_inject
  with (match_state := fun d j L1 c1 m1 L2 c2 m2 => d=c1 /\ match_cores j L1 c1 m1 L2 c2 m2 /\ 
        mem_respects_readonly ge m1 /\ mem_respects_readonly tge m2).
+ instantiate (1:= core_ord).  apply well_founded_ltof.
+ intros. destruct MS as [? [[MS [AA [BB [CC [DD EE]]]]] [MRR_SRC MRR_TGT]]]; subst; eauto. 
+ admit. (*maybe does not hold?*)
+ intros. admit. (*genv*)
+ intros. destruct MS as [? [[MS [AA [BB [CC [DD EE]]]]] [MRR_SRC MRR_TGT]]]; subst.
  split; trivial. split; trivial.
  intros. apply matchstates_inject in MS.
...
+ intros. simpl in *. destruct v; inv H; simpl.
 ...
 admit. (*TODO initial, should be as usual once we know which function pointers are preserved*) 
+ intros. destruct H0 as [? [MC [MRR_SRC MRR_TGT]]]; subst.
  exploit core_diagram; eauto.
  intros [st2' [m2' [j' [L1' [L2' [INC [SEP [LOC [[U [TSTEP HU]] MC']]]]]]]]].
  exists st2', m2', st1', j', L1', L2'. intuition.
    ...
    exploit semantics_lemmas.mem_step_readonly. eapply effstep_mem. apply TSTEP.
           intros [Fwd RD]. eapply mem_respects_readonly_fwd; eauto.
  exists U; split; trivial. left. apply effstep_plus_one. assumption.
+ intros. destruct H as [? [[MS [AA [BB [CC [DD EE]]]]] [MRR_SRC MRR_TGT]]]; subst.
  destruct c1; simpl in *; inv H0. destruct stack; inv H1. inv MS. simpl in *.
  inv STACKS.
  exists tres. intuition. 
+ intros. destruct H as [? [[MS [AA [BB [CC [DD EE]]]]] [MRR_SRC MRR_TGT]]]; subst.
  destruct c1; simpl in *; inv H0. destruct f; inv H1. inv MS. simpl in *.
  destruct (observableEF_dec e0); inv H0. intuition.
  exists targs. split; trivial. apply val_list_inject_forall_inject; trivial.
+ intros. destruct MatchMu as [? [[MS [AA [BB [CC [DD EE]]]]] [MRR_SRC MRR_TGT]]]; subst.
  destruct st1; inv AtExtSrc.
  destruct f; inv H0. simpl. inv MS. simpl in *.
  remember (observableEF_dec e0) as obs; destruct obs; inv H1. inv AtExtTgt.
  eexists; eexists; eexists. split. reflexivity. split. reflexivity. split. reflexivity.
  destruct INC as [INC SEP].
  split.
  - repeat split; trivial; intros.
    * econstructor; trivial.
      eapply match_stacks_incr_gsep; try eassumption.
               eapply match_stacks_bound. eassumption.
               eapply forward_nextblock; eassumption.
               eapply forward_nextblock; eassumption.
    * apply FwdSrc; eauto.
    * apply FwdTgt; eauto.
    * remember (j b) as q; symmetry in Heqq; destruct q.
      destruct p0. rewrite (INC _ _ _ Heqq) in H. inv H. eapply CC; eassumption.
      destruct (SEP _ _ _ Heqq H) as [X Y]; rewrite X, Y; trivial.
    * eapply Ple_trans; eauto. apply forward_nextblock; trivial.
    * eapply Ple_trans; eauto. apply forward_nextblock; trivial.
  - split; eapply mem_respects_readonly_forward'; eassumption.
Admitted. (*3 admits, all about genvs/function pointer preservation etc*)
      (*
