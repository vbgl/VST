...
Require Import minisepcomp.mini_diagram_trans_MatchInfo.
Require Import minisepcomp.interpolation_memory.
Require Import minisepcomp.mini_interpolation_MatchInfo.
...
Lemma meminj_valid_compose_meminj j1 j2 E1 E2 E3 m1 m2 m3
  (MV1: meminj_valid j1 E1 E2 m1 m2)
  (MV2: meminj_valid j2 E2 E3 m2 m3):
  meminj_valid (compose_meminj j1 j2) E1 E3 m1 m3.
Proof. destruct E1 as [M1 L1]. destruct E2 as [M2 L2]. destruct E3 as [M3 L3].
 red; intros. destruct MV1. destruct MV2. intuition.
apply compose_meminjD_Some in J. destruct J as [? [? [? [? [? ?]]]]]. subst.
eapply H2. eassumption. 
apply compose_meminjD_Some in J. destruct J as [? [? [? [? [? ?]]]]]. subst.
eapply H4. eassumption.
Qed.

Lemma EffectPropagate_compose_meminj j1 j2 M1 M2 M3
   (EP1: EffectsPropagate j1 M1 M2)
   (EP2: EffectsPropagate j2 M2 M3):
   EffectsPropagate (compose_meminj j1 j2) M1 M3.
Proof. intros MHyp b3 z Hb3.  
assert (HP1: forall b1 z, M1 b1 z = true -> j1 b1 <> None).
{ intros. specialize (MHyp _ _ H).
  unfold compose_meminj in MHyp. 
  case_eq (j1 b1); intros. discriminate.
  rewrite H0 in *; trivial. }
specialize (EP1 HP1).
assert (HP2: forall b2 z, M2 b2 z = true -> j2 b2 <> None).
{ intros b2; intros. destruct (EP1 _ _ H) as [b1 [d2 [J1 K1]]].
  specialize (MHyp _ _ K1). unfold compose_meminj in MHyp.
  rewrite J1 in MHyp. case_eq (j2 b2); intros; try discriminate. rewrite H0 in *; trivial. }
destruct (EP2 HP2 _ _ Hb3) as [b2 [d [J2 Hb2]]].
destruct (EP1 _ _ Hb2) as [b1 [d1 [J1 Hb1]]].
exists b1; unfold compose_meminj. rewrite J1, J2. eexists; split. reflexivity.
exact_eq Hb1. f_equal. f_equal. omega.
Qed.
  ...
  eapply Build_Mini_simulation_inject with
    ...
    (match_state := fun d j E1 c1 m1 E3 c3 m3 => 
      match d with (d1,X,d2) => 
        exists c2, exists m2, exists j1, exists j2, exists E2,                                 
          ...
          match_core12 d1 j1 E1 c1 m1 E2 c2 m2 /\ match_core23 d2 j2 E2 c2 m2 E3 c3 m3 /\
          full_ext j1 j2 (snd E1) (snd E2)
      end).
{ (*well_founded*)
  eapply wf_clos_trans. 
  eapply well_founded_sem_compose_ord_eq_eq; assumption. }
{ (*match_wd*) clear - match_wd12 match_wd23.
  intros. rename c2 into c3. rename m2 into m3. rename M2 into M3. rename L2 into L3. rename b2 into b3.
  destruct d as [[d12 cc2] d23].
  destruct MS as [c2 [m2 [j12 [j23 [[M2 L2] [X [JJ (*[INV*) [MC12 [MC23 FULL]]]]]]]]]; subst.
  specialize (match_wd12 _ _ _ _ _ _ _ _ _ _ MC12).
  specialize (match_wd23 _ _ _ _ _ _ _ _ _ _ MC23).
...
{ (*match_genv*)
  ...
  intros. rename c2 into c3. rename m2 into m3. rename E2 into E3.
  destruct d as [[d12 cc2] d23].
  destruct MC as [c2 [m2 [j12 [j23 [E2 [X [J (*[INV*) [MC12 [MC23 IFDL]]]]]]]]]; subst.
...
{ (*meminj_valid*)
    ...
    intros. rename c2 into c3. rename m2 into m3. rename E2 into E3.
    destruct d as [[d12 cc2] d23].
    destruct MS as [c2 [m2 [j12 [j23 [E2 [X [J (*[INV*) [MC12 [MC23 IFDL]]]]]]]]]; subst.
    specialize (match_valid12 _ _ _ _ _ _ _ _ MC12).
    specialize (match_valid23 _ _ _ _ _ _ _ _ MC23).
    unfold meminj_valid.
    destruct E1 as [M1 L1]. destruct E3 as [M3 L3]. destruct E2 as [M2 L2].
    destruct match_valid12 as [MC12a [MC12b [MC12c [MC12d MC12e]]]].
    destruct match_valid23 as [MC23a [MC23b [MC23c [MC23d MC23e]]]].
    split; intros. eauto.
    split; intros. eauto.
    split; intros. rename b2 into b3. apply compose_meminjD_Some in J.
...
(*  destruct (core_initial12 _ _ _ _ _ vals2 _ 
  ...
  exists c3. 
  split; trivial. 
  exists c2, m2, j1, j2, EmptyInfo. intuition.
...
{ (*effcore_diagram*)
   clear - match_wd12 match_valid12 effcore_diagram12 
           ...
           match_genv23.
   intros. rename st2 into st3. rename m2 into m3. rename E2 into E3.
   destruct cd as [[d12 cc2] d23].
   destruct H0 as [c2 [m2 [j12 [j23 [E2 [X [J (*[INV*) [MC12 [MC23 full]]]]]]]]]; subst.
...
{ (*halted*)
  ...
  intros. rename c2 into c3. rename m2 into m3. rename L2 into L3. rename M2 into M3.  
  destruct cd as [[d12 cc2] d23].
  destruct H as [c2 [m2 [j12 [j23 [[M2 L2] [X [J (*[INV*) [MC12 [MC23 full]]]]]]]]]; subst.
  destruct (core_halted12 _ _ _ _ _ _ _ _ _ _ _ MC12 H0) as
     [v2 [MInj12 [MRR1 [MRR2 [RValsInject12 [HaltedMid EProp12]]]]]]. 
  destruct (core_halted23 _ _ _ _ _ _ _ _ _ _ _ MC23 HaltedMid) as
     [v3 [MInj23 [_ [MRR3 [RValsInject23 [HaltedTgt EProp23]]]]]].
  exists v3.
  assert (WDmu12:= match_wd12 _ _ _ _ _ _ _ _ _ _ MC12).
...
(*  destruct INV as [INVa [INVb [INVc INVd]]].*)
  ...
  split. (* rewrite compose_sm_as_inj; trivial.
         ...
         apply restrict_incr.*)
  split. assumption.
  eapply EffectPropagate_compose_meminj; eassumption. } 
{ (*at_external*)
  clear - match_wd12 core_at_external12 match_wd23 core_at_external23.
  intros. rename c2 into c3. rename m2 into m3. rename L2 into L3. rename M2 into M3.
  rename H0 into AtExtSrc. 
  destruct cd as [[d12 cc2] d23]. 
  destruct H as [st2 [m2 [j12 [j23 [[M2 L2] [Hst2 [J (*[GLUEINV*) [MC12 [MC23 full]]]]]]]]]. 
  subst.
  destruct (core_at_external12 _ _ _ _ _ _ _ _ _ _ _ _ _ MC12 AtExtSrc)
    as [MInj12 [MRR1 [MRR2 [EProp12 [vals2 [ArgsInj12 AtExt2]]]]]]; 
     clear core_at_external12.
  destruct (core_at_external23 _ _ _ _ _ _ _ _ _ _ _ _ _ MC23 AtExt2)
    as [MInj23 [_ [MRR3 [EProp23 [vals3 [ArgsInj23 AtExtTgt]]]]]]; 
     clear core_at_external23.
    split. eapply Mem.inject_compose; eassumption.
    split; trivial.
    split; trivial.
    split. eapply EffectPropagate_compose_meminj; eassumption. 
    exists vals3.
...
{ (*after_external*)
  ...
  intros. rename st2 into st3. rename m2 into m3. rename L2 into L3. rename M2 into M3. 
          rename vals2 into vals3'. rename m2' into m3'.
          rename RDO2 into RDO3. rename ret2 into ret3.
  destruct cd as [[d12 cc2] d23]. 
  destruct MatchMu as [st2 [m2 [j12 [j23 [[M2 L2] [Hst2 [HMu (*[GLUEINV*) [MC12 [MC23 full]]]]]]]]].
  assert (WDmu12:= match_wd12 _ _ _ _ _ _ _ _ _ _ MC12).
  assert (WDmu23:= match_wd23 _ _ _ _ _ _ _ _ _ _ MC23).
(*
...
(*  assert (NormMC12: match_core12 d12 j12 L1 st1 m1 L2 st2 m2) by assumption.
  ...
  destruct (core_at_external12 _ _ _ _ _ _ _ _ _ _ _ _ _ MC12 AtExtSrc)
   as [MInj12 [MRR1 [MRR2 [EProp12 [vals2 [ArgsInj12 (*[ArgsHT2*) AtExt2(*]*)]]]]]]; clear core_at_external12.
  destruct (core_at_external23 _ _ _ _ _ _ _ _ _ _ _ _ _ MC23 AtExt2)
   as [MInj23 [_ [MRR3 [EProp23 [vals3 [ArgsInj23 (*[ArgsHT3*) AtExt3(*]*)]]]]]]; clear core_at_external23.
  ...
  specialize (eff_after_external12 _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ MInj12 
        MC12 AtExtSrc AtExt2 ArgsInj12). 
  specialize (eff_after_external23 _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ MInj23
  ...
  exploit (interp_II m1 m2 j12 L1 L2 L3). eassumption. eassumption. eassumption. eassumption. eassumption.
  ...
  split. assumption.
  split. assumption.
  exists c2', m2', j12', j23', (M2,L2). simpl.
  split. trivial.
  split. trivial.
  split. assumption.
  split; assumption. 
