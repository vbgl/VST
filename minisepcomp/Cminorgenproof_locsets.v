...
Lemma match_callstack_match_globalenvs:
  ...
  match_globalenvs f hi /\ Ple hi tbound /\ Ple hi (Mem.nextblock m) /\ Ple hi (Mem.nextblock tm) /\
  forall b (Hb: Plt b hi), L b = false /\ TL b = false.
Proof.
  induction 1; eauto. destruct IHmatch_callstack as [? [? [? [? ?]]]].
  intuition. xomega. 
Qed.
...
Proof.
  ...
  + eapply match_callstack_invariant with (tm1 := tm); eauto; try xomega.
    exploit match_callstack_match_globalenvs; eauto. intros [MG [VB1 [VB2 [VB3 HH]]]].
...
Proof.
  ...
  { rewrite (freshblock_alloc _ _ _ _ _ H).
    ...
    intros bb; split; intros Hbb. rewrite Hbb; trivial. 
    destruct (HH _ Hbb). rewrite H9; simpl. apply freshblock_charF. left; unfold Mem.valid_block; xomega. 
    intros bb; split; intros Hbb; trivial. apply HH; trivial.
...
(*CompComp adaptation: state refactoring; added parameters L TL h*)
Inductive match_states L TL h: Csharpminor_memsem.state -> mem -> Cminor_memsem.state -> mem -> Prop :=
  | match_state:
      ...
      match_states L TL h (Csharpminor_memsem.State fn s k e le) m
  ...
  | match_state_seq:
      ...
      match_states L TL h (Csharpminor_memsem.State fn (Csharpminor_memsem.Sseq s1 s2) k e le) m
  ...
  | match_callstate:
      ...
      match_states L TL h (Csharpminor_memsem.Callstate fd args k) m
  ...
  | match_returnstate:
      ...
      match_states L TL h (Csharpminor_memsem.Returnstate v k) m
...
Lemma match_is_call_cont:
  ...
  exists tk',
    corestep_star CMin_memsem tge (State tfn Sskip tk sp te) tm
...
Proof.
  ...
  eapply corestep_star_trans. apply corestep_star_one; constructor. eassumption.
  auto.
  econstructor; split. apply corestep_star_zero. split. exact I. econstructor; eauto.
Qed.
...
Lemma switch_descent:
  ...
  transl_lblstmt_cont cenv xenv ls k k'
  /\ (forall f sp e m,
      corestep_plus CMin_memsem tge (State f s k sp e) m (State f body k' sp e) m).
Proof.
  induction ls; intros.
- monadInv H. econstructor; split.
  econstructor.
  intros. eapply corestep_plus_trans; eapply corestep_plus_one; constructor.
- monadInv H. exploit IHls; eauto. intros [k' [A B]].
  econstructor; split.
  econstructor; eauto.
  intros. eapply corestep_plus_star_trans. eauto.
  eapply corestep_star_trans; eapply corestep_star_one; constructor.
Qed.
...
Lemma switch_ascent:
  ...
  exists k2,
  corestep_star CMin_memsem tge (State f (Sexit n) k1 sp e) m
             (State f (Sexit O) k2 sp e) m
  /\ transl_lblstmt_cont cenv xenv ls' k k2.
Proof.
  induction 1; intros.
- exists k1; split; auto. apply corestep_star_zero.
- inv H0. exploit IHlbl_stmt_tail; eauto. intros (k2 & P & Q).
  ...
  eapply corestep_star_trans. eapply corestep_star_one; econstructor. eexact P.
Qed.
...
Lemma switch_match_states:
  ...
  exists S,
  corestep_plus CMin_memsem tge (State tfn (Sexit O) tk' (Vptr sp Int.zero) te) tm S tm
  /\ match_states L TL h (Csharpminor_memsem.State fn (seq_of_lbl_stmt ls) k e le) m S tm.
Proof.
  intros. inv TK.
- econstructor; split.
    eapply corestep_plus_trans; eapply corestep_plus_one; econstructor. 
  eapply match_state; eauto.
- econstructor; split. 
    eapply corestep_plus_trans; eapply corestep_plus_one; constructor. 
...
(*CompComp adaptation: state refactoring, removed trace*)
Lemma transl_step_correct:
  forall S1 m1 S2 m2, Csharpminor_memsem.step ge S1 m1 S2 m2 ->
  forall T1 tm1 L TL h, match_states L TL h S1 m1 T1 tm1 ->
  (exists T2 tm2, corestep_plus CMin_memsem tge T1 tm1 T2 tm2 /\ 
                  match_states (fun b => L b || freshblock m1 m2 b) 
                               (fun b => TL b || freshblock tm1 tm2 b) 
                                h S2 m2 T2 tm2)
  \/ (measure S2 < measure S1 /\ match_states (fun b => L b || freshblock m1 m2 b) 
                                              TL h S2 m2 T1 tm1)%nat.
Proof.
  induction 1; intros T1 tm1 L TL h MSTATE; inv MSTATE; try rewrite freshblock_irrefl', orb_false_r_ext.

- (* skip seq *)
  monadInv TR. left.
  dependent induction MK.
  + eexists; econstructor; split.
    apply corestep_plus_one. constructor. rewrite freshblock_irrefl', orb_false_r_ext.
    econstructor; eauto. 
  + eexists; econstructor; split.
    apply corestep_plus_one. constructor. rewrite freshblock_irrefl', orb_false_r_ext.
    eapply match_state_seq; eauto.
  + exploit IHMK; eauto. intros [T2 [tm2 [A B]]].
    exists T2; eexists; split. eapply corestep_one_plus_plus. constructor. eauto. 
    auto.
- (* skip block *)
  monadInv TR. left.
  dependent induction MK.
...
- (* skip call *)
  ...
  eexists; econstructor; split.
  eapply corestep_star_plus_trans. eexact A. apply corestep_plus_one. apply step_skip_call. auto. eauto.
  erewrite (freshblock_free _ _ _ _ _ P), orb_false_r_ext.
  erewrite (freshblock_free_list _ _ _ H0), orb_false_r_ext.
  econstructor; eauto.
...
- (* store *)
  ...
  apply corestep_plus_one. econstructor; eauto.
  econstructor; eauto.
  erewrite freshblock_storev, orb_false_r_ext; eauto.
  erewrite freshblock_storev, orb_false_r_ext; eauto.
  ...
  eapply match_callstack_invariant with f0 m tm1; eauto.
...
- (* call *)
  ...
  left; eexists; econstructor; split.
  apply corestep_plus_one. eapply step_call; eauto.
  apply sig_preserved; eauto. rewrite freshblock_irrefl', orb_false_r_ext.
  econstructor; eauto.
...
- (* builtin *)
  ...
  exploit transl_exprlist_correct; eauto.
  intros [tvargs [EVAL2 VINJ2]].
  exploit match_callstack_match_globalenvs; eauto. intros [hi' [MG [HH1 [HH2 HH3]]]].
  ...
  { eapply match_callstack_sub.
    + apply match_callstack_incr_bound with (Mem.nextblock m) (Mem.nextblock tm1).
      ...
      eapply external_call_nextblock; eauto.
    + intros bb; split; intros Hbb. rewrite Hbb; trivial. destruct (HH3 _ Hbb) as [A _]; rewrite A; simpl. apply freshblock_charF. left. unfold Mem.valid_block; xomega.
    + intros bb; split; intros Hbb. rewrite Hbb; trivial. destruct (HH3 _ Hbb) as [_ A]; rewrite A; simpl. apply freshblock_charF. left. unfold Mem.valid_block; xomega. }
  econstructor; eauto.
Opaque PTree.set.
...
- (* seq *)
  ...
  econstructor; eauto.
- (* seq 2 *)
  right. split. auto. econstructor; eauto.

- (* ifthenelse *)
  monadInv TR.
  exploit transl_expr_correct; eauto. intros [tv [EVAL VINJ]].
  left; exists (State tfn (if b then x0 else x1) tk (Vptr sp Int.zero) te); eexists; split.
  apply corestep_plus_one. eapply step_ifthenelse; eauto. eapply bool_of_val_inject; eauto.
  rewrite freshblock_irrefl', orb_false_r_ext. econstructor; eauto. destruct b; auto.
...
- (* exit seq *)
  ...
  exploit IHMK; eauto. intros [T2 [tm2 [A B]]].
  exists T2; eexists; split; auto. eapply corestep_one_plus_plus. constructor. eauto. eauto.
  exploit IHMK; eauto. intros [T2 [tm2 [A B]]].
  exists T2; eexists; split; auto. eapply corestep_one_plus_plus.
...
- (* exit block 0 *)
  ...
  dependent induction MK.
  eexists; econstructor; split.
  simpl. apply corestep_plus_one. constructor. rewrite freshblock_irrefl', orb_false_r_ext.
  econstructor; eauto.
  exploit IHMK; eauto. intros [T2 [tm2 [A B]]].
  exists T2; eexists; split; auto. simpl.
  eapply corestep_one_plus_plus. constructor. eauto. eauto. 
...
- (* exit block n+1 *)
  ...
  exploit IHMK; eauto. intros [T2 [tm2 [A B]]].
  exists T2; eexists; split; auto. simpl.
  eapply corestep_one_plus_plus. constructor. eauto. auto. 
...
- (* switch *)
  ...
  left; exists T2, tm1; split.
  eapply corestep_plus_star_trans. eapply B.
  eapply corestep_star_trans. apply corestep_star_one. econstructor; eauto.
  eapply corestep_star_trans. eexact C. eapply corestep_star_plus. eexact F.
  rewrite freshblock_irrefl', orb_false_r_ext. auto.

- (* return none *)
  monadInv TR. left.
  exploit match_callstack_freelist; eauto. intros [tm' [A [B C]]].
  eexists; econstructor; split.
  apply corestep_plus_one. eapply step_return_0. eauto.
...
- (* return some *)
  ...
  exploit match_callstack_freelist; eauto. intros [tm' [A [B C]]].
  eexists; econstructor; split.
  apply corestep_plus_one. eapply step_return_1. eauto. eauto.
  erewrite freshblock_free_list, orb_false_r_ext; eauto.
  erewrite freshblock_free, orb_false_r_ext; eauto.
  econstructor; eauto. eapply match_call_cont; eauto.
...
- (* label *)
  ...
  rewrite freshblock_irrefl', orb_false_r_ext. econstructor; eauto.
...
- (* goto *)
  ...
  intros [ts' [tk' [xenv' [A [B C]]]]].
  left; econstructor; eexists; split.
  apply corestep_plus_one. apply step_goto. eexact A.
  rewrite freshblock_irrefl', orb_false_r_ext. econstructor; eauto.
...
- (* internal call *)
  ...
  left; econstructor; eexists; split.
  apply corestep_plus_one. constructor; simpl; eauto.
...
- (* external call *)
  ...
  intros [f' [vres' [tm' [EC [VINJ [MINJ' [UNMAPPED [OUTOFREACH [INCR SEPARATED]]]]]]]]].
  left; econstructor; eexists; split.
  apply corestep_plus_one. econstructor. trivial.
  eapply external_call_symbols_preserved; eauto. apply senv_preserved.
  econstructor; eauto.
  eapply match_callstack_sub.
  { apply match_callstack_incr_bound with (Mem.nextblock m) (Mem.nextblock tm1).
...
- (* return *)
  inv MK. simpl.
  left; econstructor; eexists; split.
  apply corestep_plus_one. econstructor; eauto. rewrite freshblock_irrefl', orb_false_r_ext.
...
Lemma transl_initial_states:
  ...
  exists R, CMin_initial_core tge tv tvals = Some R /\
            match_states (fun b=> false) (fun b => false) bound S m R tm.
Proof.
...
Lemma transl_final_states:
  forall L TL h S m R tm r,
  match_states L TL h S m R tm -> CSharpMin_halted S = Some r ->
  exists j tr, CMin_halted R = Some tr /\ Mem.inject j m tm /\
               Val.inject j r tr.
Proof.
  intros. destruct S; inv H0. destruct k; inv H2. inv H. simpl.
  inv MK. exists f, tv; intuition. 
Qed.
(*
Lemma transl_final_states:
  forall S R r,
...
Lemma transl_atExt_states:
  forall L TL h S m R tm f sg args,
  match_states L TL h S m R tm -> CSharpMin_at_external S = Some (f, sg, args) ->
  exists j targs, CMin_at_external R = Some (f, sg, targs) /\ Mem.inject j m tm /\
...
Lemma transl_afterExt_states:
  ...
  exists Q P,
    ...
    match_states L TL h Q m2 P tm2.
Proof.
  intros. destruct S; inv H0. destruct f0; inv H2. inv H. simpl. inv TR.
  destruct (BuiltinEffects.observableEF_dec e); inv H1. rename f0 into j.
  exists j, targs; intuition. simpl.
  eexists; eexists; split. reflexivity. split. reflexivity.
  econstructor; try eassumption.
  eapply match_callstack_incr_bound; try eassumption.
  eapply match_callstack_external_call; try eassumption; xomega.
Qed.

End TRANSLATION. (*CompCert 2.7.1 has 2233 lines, and Cminorgenproof_4_memsem has 2353 lines,
so adding L, TL, and h has added another 70 lines or so.*)
