...
Require Import Coqlib Maps Integers Floats minisepcomp.Lattice minisepcomp.Kildall.
Require Import minisepcomp.Compopts AST Linking.
Require Import Values Memory Globalenvs Events.
Require Import minisepcomp.Registers minisepcomp.Op minisepcomp.RTL_memsem.
Require Import minisepcomp.ValueDomain minisepcomp.ValueAOp minisepcomp.Liveness.
...
(*CompComp adaptation: added parameter L, ensure sp is local*)
Inductive sound_stack (L: block -> bool): block_classification -> list stackframe -> mem -> block -> Prop :=
  | sound_stack_nil: forall bc m bound,
      sound_stack L bc nil m bound
  | sound_stack_public_call:
      forall (bc: block_classification) res f sp pc e stk m bound bc' bound' ae
        (STK: sound_stack L bc' stk m sp)
      ...
      sound_stack L bc (Stackframe res f (Vptr sp Int.zero) pc e :: stk) m bound
...
(*CompComp adaptation: refactoring of RTL.state, added parameter L*)
Inductive sound_state_base (L: block -> bool): state -> mem -> Prop :=
  | sound_regular_state:
      forall s f sp pc e m ae am bc
        (STK: sound_stack L bc s m sp)
        ...
        (SP: bc sp = BCstack)
        (*compcomp new*)(Lsp: L sp = true),
      sound_state_base L (State s f (Vptr sp Int.zero) pc e) m
  | sound_call_state:
      forall s fd args m bc
        (STK: sound_stack L bc s m (Mem.nextblock m))
        (ARGS: forall v, In v args -> vmatch bc v Vtop)
        (RO: romatch bc m rm)
        (MM: mmatch bc m mtop)
        (GE: genv_match bc ge)
        (NOSTK: bc_nostack bc),
      sound_state_base L (Callstate s fd args) m
  | sound_return_state:
      forall s v m bc
        (STK: sound_stack L bc s m (Mem.nextblock m))
        ...
        (GE: genv_match bc ge)
        (NOSTK: bc_nostack bc),
      sound_state_base L (Returnstate s v) m.

(** Properties of the [sound_stack] invariant on call stacks. *)

Lemma sound_stack_ext:
  forall L m' bc stk m bound,
  sound_stack L bc stk m bound ->
...
Lemma sound_stack_storev:
  forall L chunk m addr v m' bc aaddr stk bound,
...
Lemma sound_stack_storebytes:
  forall L m b ofs bytes m' bc aaddr stk bound,
...
Lemma sound_stack_free:
  forall L m b lo hi m' bc stk bound,
...
Lemma sound_stack_exten:
  forall L bc stk m bound (bc1: block_classification),
  sound_stack L bc stk m bound ->
  (forall b, Plt b bound -> bc1 b = bc b) ->
  sound_stack L bc1 stk m bound.
Proof.
  intros. inv H.
- constructor.
- assert (Plt sp bound') by eauto with va.
...
(*CompComp adaptation: refactoring of RTL_state*)
Lemma sound_succ_state:
  forall L bc pc ae am instr ae' am'  s f sp pc' e' m',
  ...
  genv_match bc ge ->
  bc sp = BCstack ->
  sound_stack L bc s m' sp ->
  (*compcomp new*) L sp = true ->
  sound_state_base L (State s f (Vptr sp Int.zero) pc' e') m'.
Proof.
...
Lemma sound_stack_sub: forall L bc s m sp (SS:sound_stack L bc s m sp)
  LL (HLL: forall b, L b = true -> LL b = true), sound_stack LL bc s m sp.
Proof. induction 1; simpl; intros.
eapply sound_stack_nil; eauto.
eapply sound_stack_public_call; eauto.
eapply sound_stack_private_call; eauto.
Qed.

Lemma sound_state_base_sub L st m (SSB: sound_state_base L st m)
      LL (HLL: forall b, L b = true -> LL b = true): sound_state_base LL st m.
Proof. inv SSB; econstructor; eauto; eapply sound_stack_sub; eauto. Qed.

(*CompComp adaptation: refactoring of RTL_state, use effstep, replaced trace t by effect E.
In the proof, numbering of hypotheses off by 1 for Hi (i>1)*)
Theorem sound_step_base:
  forall L st m E st' m', effstep RTL_eff_sem ge E st m st' m' -> sound_state_base L st m -> 
  sound_state_base (fun b => L b || freshblock m m' b) st' m'.
Proof.
...
* (* private builtin call *)
  InvBooleans. rewrite forallb_forall in (*H3*)H4.
  exploit hide_stack; eauto. apply pincl_ge; auto.
  intros (bc1 & A & B & C & D & E & F & G).
  exploit external_call_match; eauto.
  intros. exploit list_forall2_in_l; eauto. intros (av & U & V).
  eapply D; eauto with va. apply vpincl_ge. (*apply H3; auto.*) apply H4; auto.
  intros (bc2 & J & K & LL & M & N & O & P & Q).
...
* (* public builtin call *)
  ...
  (*new*)eapply sound_state_base_sub with (L:=L). 2: intros b Hb; rewrite Hb; trivial.
  unfold transfer_builtin in TR.
  destruct ef; auto. 
+ (* volatile load *)
  inv H0; auto. (*inv H3*) inv H4; auto. inv H1. 
  ...
  * (* true volatile access *)
    ...
    { inv (*H4*)H5; simpl in *; constructor. econstructor. eapply GE; eauto. }
...
+ (* volatile store *)
  inv H0; auto. inv (*H3*)H4; auto. inv (*H4*)H5; auto. inv H1.
  exploit abuiltin_arg_sound. eauto. eauto. eauto. eauto. eauto. eexact H0. intros VM1.
  exploit abuiltin_arg_sound. eauto. eauto. eauto. eauto. eauto. eexact (*H2*)H3. intros VM2.
  inv (*H9*)H10.
  * (* true volatile access *)
    eapply sound_succ_state; eauto. simpl; auto.
...
+ (* annot val *)
  inv H0; auto. inv (*H3*)H4; auto. inv H1.
  eapply sound_succ_state; eauto. simpl; auto.
...
- (* cond *)
  ...
  unfold transfer; rewrite H; auto.
    eapply sound_stack_sub; try eassumption. intros bb Hbb; rewrite Hbb; trivial.
    rewrite Lsp; trivial.
...
- (* internal function *)
  ...
  rewrite (freshblock_alloc _ _ _ _ _ H). destruct (eq_block stk stk); simpl; [ apply orb_true_r | congruence]. 
- (* external function *)
...
(*CompComp adaptation: state refactoring, add L*)
Inductive sound_state L: state -> mem-> Prop :=
  | sound_state_intro: forall st m,
      (forall cunit, linkorder cunit prog -> sound_state_base cunit ge L st m) ->
      sound_state L st m.

(*CompComp adaptation: state refactoring, use effsem, replace trace t by E, add L evolution*)
Theorem sound_step:
  forall L st m E st' m', effstep RTL_eff_sem ge E st m st' m' -> sound_state L st m -> 
  sound_state (fun b => L b || freshblock m m' b) st' m'.
Proof.
...
Remark sound_state_inv:
  forall L st m cunit,
  sound_state L st m -> linkorder cunit prog -> sound_state_base cunit ge L st m.
Proof.
  intros. inv H. eauto. 
Qed.
...
(*CompComp adaptation: eliminated lemma - initial_state now treated differently - 
  maybe reestablish for initcore?
Theorem sound_initial:
  forall prog st m, initial_state prog st m -> sound_state prog st m.
Proof.
  destruct 1.
...
(*CompComp adaptation: state refactoring, add ?L and one more underscore in (sound_state_inv _ _ _ _ _ H1 H2)*)
Ltac InvSoundState :=
  match goal with
  | H1: sound_state ?prog ?L ?st ?m, H2: linkorder ?cunit ?prog |- _ =>
...
(*CompComp adaptation: state refactoring, add L*)
Lemma avalue_sound:
  forall L cunit prog s f sp pc e m r,
  sound_state prog L (State s f (Vptr sp Int.zero) pc e) m ->
...
(*CompComp adaptation: state refactoring, add L*)
Lemma aaddr_sound:
  forall L cunit prog s f sp pc e m r b ofs,
  sound_state prog L (State s f (Vptr sp Int.zero) pc e) m ->
  linkorder cunit prog ->
  e#r = Vptr b ofs ->
  exists bc,
...
Lemma aaddressing_sound:
  forall L cunit prog s f sp pc e m addr args b ofs,
  sound_state prog L (State s f (Vptr sp Int.zero) pc e) m ->
  linkorder cunit prog ->
...
Lemma aaddr_arg_sound:
  forall L cunit prog s f sp pc e m a b ofs,
  sound_state prog L (State s f (Vptr sp Int.zero) pc e) m ->
  linkorder cunit prog ->
