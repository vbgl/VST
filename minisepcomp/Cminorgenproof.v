...
(*CompComp: added parameter L and TL, strengthened mi_incr and added me_local*)
Record match_env (f: meminj) (L TL: block -> bool) (cenv: compilenv)
...
(** All C#minor blocks below [lo] (i.e. allocated before the blocks
  ...
  (i.e. allocated before the stack data for the current Cminor function). *)
    me_incr:
      forall b tb delta,
      f b = Some(tb, delta) -> Plt b lo -> Plt tb sp (*/\ (*new*) L b = false /\ TL tb = false*);

(*Compcomp adpatation: assert that stackblocks are local in Src and Tgt*)
    me_local:
      TL sp = true /\ (forall id b sz, PTree.get id e = Some(b, sz) -> L b = true);
  }.
...
Lemma match_env_invariant:
  forall f1 L TL cenv e sp lo hi f2,
  match_env f1 L TL cenv e sp lo hi ->
  inject_incr f1 f2 ->
  (forall b delta, f2 b = Some(sp, delta) -> f1 b = Some(sp, delta)) ->
  (forall b, Plt b lo -> f2 b = f1 b) ->
  match_env f2 L TL cenv e sp lo hi.
Proof.
...
Qed.
Lemma match_env_invariant':
  forall f1 L TL cenv e sp lo hi f2 m,
  match_env f1 L TL cenv e sp lo hi ->
  mini_extern_incr f1 f2 L TL ->
  globals_separate m tge f1 f2 ->
  Ple lo (Mem.nextblock m) ->
 (* (forall b delta, f2 b = Some(sp, delta) -> f1 b = Some(sp, delta)) ->*)
(*  (forall b, L b = true -> f2 b = f1 b) ->*)
  match_env f2 L TL cenv e sp lo hi.
Proof.
  intros. destruct H0 as [INC SEP]. destruct H. constructor; auto.
(* vars *)
  intros. geninv (me_vars0 id); econstructor; eauto.
(* bounded *)
  intros. case_eq (f1 b); intros. destruct p. rewrite (INC _ _ _ H0) in H. inv H. eauto.
  destruct (SEP _ _ _ H0 H). destruct me_local0;  congruence. 
(* below *)
  intros. case_eq (f1 b); intros. destruct p. rewrite (INC _ _ _ H3) in H. inv H. eauto.
  destruct (SEP _ _ _ H3 H). destruct (H1 _ _ _ H3 H). elim H6; clear -H0 H2. unfold Mem.valid_block; xomega. 
Qed.
...
(*CompComp adaptation: this lemma is now only used for builtins etc, since
 AtExt/afterExt interaction's will not satisfy the inject_separated assumption*)
Lemma match_env_external_call:
  forall f1 L TL cenv e sp lo hi f2 m1 m1',
  match_env f1 L TL cenv e sp lo hi ->
  inject_incr f1 f2 ->
...
(* CompComp adaptation: the variant of match_env_external_call
   suitable fopr At/After_external interactions*) 
Remark inject_incr_sep_same:
  forall f1 f2 m1,
  inject_incr f1 f2 -> globals_separate m1 tge f1 f2 ->
  forall b, Mem.valid_block m1 b -> f2 b = f1 b.
Proof.
  intros. case_eq (f1 b).
  + clear H0. intros [b' delta] EQ. apply H; auto.
  + intros EQ. case_eq (f2 b); trivial.
    intros [b'1 delta1] EQ1. exploit H0; eauto. intros [C D]. contradiction.
Qed.

Lemma match_env_external_AfterExt:
  forall f1 L TL cenv e sp lo hi f2 m1 m1',
  match_env f1 L TL cenv e sp lo hi ->
  mini_extern_incr f1 f2 L TL ->
  globals_separate m1 tge f1 f2 ->
  Ple hi (Mem.nextblock m1) -> Plt sp (Mem.nextblock m1') ->
  match_env f2 L TL cenv e sp lo hi.
Proof.
  intros. destruct H0 as [INC SEP]. apply match_env_invariant with f1; auto.
  { intros. remember (f1 b) as d; symmetry in Heqd; destruct d.
    + destruct p. rewrite (INC _ _ _ Heqd) in H0; trivial.
    + destruct (SEP _ _ _ Heqd H0). destruct H. destruct me_local0. congruence. }
  { intros. eapply inject_incr_sep_same; eauto. red. destruct H. xomega. }
Qed. 
(*******************************************************)
...
Lemma match_env_alloc:
  forall f1 L TL id cenv e sp lo m1 sz m2 b ofs f2,
  match_env f1 L TL (PTree.remove id cenv) e sp lo (Mem.nextblock m1) ->
  ...
  e!id = None ->
  match_env f2 (fun b => L b || freshblock m1 m2 b) TL cenv (PTree.set id (b, sz) e) sp lo (Mem.nextblock m2).
Proof.
...
(* incr *)
  intros. rewrite OTHER in H. eauto. (*destruct (me_incr0 _ _ _ H H0) as [A [B C]].
  rewrite B; repeat split; trivial; try xomega; simpl. apply freshblock_charF.
    left. unfold Mem.valid_block in *. xomega.
  intros N; subst. unfold Mem.valid_block in *. xomega.*)
  unfold block in *; xomega.
(*new local*)
  destruct me_local0; split; trivial; intros. specialize (H0 id0 b sz0).
  rewrite PTree.gsspec in H1. rewrite (freshblock_alloc _ _ _ _ _ ALLOC).
  destruct (peq id0 id); subst. inv H1. 
  destruct (eq_block (Mem.nextblock m1) (Mem.nextblock m1)). apply orb_true_r. elim n; trivial.
  rewrite H0; trivial.
Qed.
...
Lemma padding_freeable_invariant:
  forall f1 L TL e tm1 sp sz cenv lo hi f2 tm2,
...
Inductive match_callstack (L TL: block -> bool) (f: meminj) (m: mem) (tm: mem):
  ...
  | mcs_cons:
      forall cenv tf e le te sp lo hi cs bound tbound h
        ...
        (MENV: match_env f L TL cenv e sp lo hi)
...
Lemma match_env_sub f L TL cenv e sp lo hi h (M:match_env f L TL cenv e sp lo hi)
      K (HK: forall b, (L b = true -> K b = true))
      TK (HTK: forall b, (TL b = true -> TK b = true) /\
                        (Plt b h -> TK b = false)):
      match_env f K TK cenv e sp lo hi.
Proof. destruct M; constructor; eauto; intros.
destruct me_local0. split. apply HTK; trivial. intros; apply HK; eauto. 
Qed.
...
+ econstructor; eauto.
  eapply match_env_sub; try eassumption. apply HK. apply HTK; trivial. 
Qed.
...
(*  inject_separated f1 f2 m1 m1' ->*)  globals_separate m1 tge f1 f2 ->
  ...
  (*new*) (forall b1 b2 delta, f1 b1 = None -> f2 b1 = Some (b2, delta) -> Plt b2 h -> Plt b1 h) ->
...
+ (* base case *)
  ...
  intro EQ. (*rewrite (GESP _ _ _ H) in EQ. discriminate. xomega. *)
  specialize (H3 _ _ _ EQ H H6).
  destruct (GSEP _ _ _ EQ H). elim H7; clear H7. unfold Mem.valid_block; xomega.
...
+ (* inductive case *)
  ...
  - eapply match_env_invariant'; eauto.
    split; trivial.
(*    { red. intros. destruct (f1 b) as [[b' delta']|] eqn:?.
    * exploit INCR; eauto. intros. rewrite H5 in H4; trivial. (* congruence.*)
    * destruct (SEPARATED _ _ _ Heqo H4). congruence. 
    * *)intros. exploit match_callstack_match_globalenvs; eauto. intros [MG [A1 [A2 [A3 [A4 A5]]]]].
      inv MENV. destruct me_local0. xomega. (*
      case_eq (f1 b).
      intros [b2' delta'] EQ. apply INCR; trivial.
      intro EQ. case_eq (f2 b); trivial. intros. destruct p. exfalso.
      destruct (GSEP _ _ _ EQ H7) as [AA BB]. elim AA; clear AA. unfold Mem.valid_block. xomega.
     *)
  ...
  (*  exploit match_callstack_match_globalenvs; eauto. intros [MG [A1 [A2 [A3 [A4 A5]]]]].
    ...
    * exploit INCR; eauto. intros. rewrite H5 in H4; trivial. (* congruence.*)
    * rewrite H0 in H4. congruence. inv MENV. destruct me_local0.
       xomega. (* elim B. red. xomega.
...
+ constructor; intros.
  ...
  - (*new: me_local*)  split.
    ...
    intros. rewrite PTree.gempty in H4. discriminate.
...
Lemma match_callstack_alloc_left:
  ...
  (forall b', b' <> b -> f2 b' = f1 b') ->
  e!id = None ->
  match_callstack (fun b => L b || freshblock m1 m2 b) TL f2 m2 tm h
...
+ eapply match_callstack_sub.
  ...
  - intros; split; intros. rewrite H; trivial.
    exploit match_callstack_match_globalenvs; try eassumption. intros [MG [A1 [A2 [A3 [A4 A5]]]]].
    destruct (A5 _ H) as [AA BB]; rewrite AA; simpl.
    apply freshblock_charF. left. unfold Mem.valid_block; xomega.
  - intros; split; intros; trivial.
    exploit match_callstack_match_globalenvs; try eassumption. intros [MG [A1 [A2 [A3 [A4 A5]]]]]. eapply A5; trivial.
+ trivial.
Qed.
...
Lemma freshblock_trans':
  forall m m'' m' : mem,
  mem_forward m m'' ->
  mem_forward m'' m' -> (fun b => freshblock m m'' b || freshblock m'' m' b) = freshblock m m'.
Proof. intros.
  extensionality b. apply freshblock_trans; eauto. Qed.
...
Lemma match_callstack_alloc_variables_rec:
  forall h tm sp tf cenv le te lo cs
  ...
  /\ (*new*) globals_separate m1 tge f1 f2.
Proof.
  ...
  induction 1; intros f1 L TL NOREPET COMPAT SEP1 SEP2 UNBOUND MCS MINJ;
...
(* exploit Mem.alloc_result; eauto. intros RES.
  ...
  intros [f2 [A [B [C D]]]].
  exploit (IHalloc_variables f2 (fun b : block => L b || freshblock m m1 b) TL); eauto.
  ...
  (*new*) intros [f3 [MSTK [MEMINJ [[INCA INCB] SEP]]]].
     specialize (alloc_forward _ _ _ _ _ H); intros Fwd1.
     apply alloc_variables_mem_step in H0. apply mem_step_forward in H0. 
     exists f3. split.
     - exploit match_callstack_match_globalenvs. eapply MCS. intros [MG [VM [VTM [_ [_ ZZ]]]]].
       eapply match_callstack_sub; try eassumption.
       * intros. erewrite <- freshblock_trans with (m'':=m1), orb_assoc. split; trivial.
         intros. destruct (ZZ _ H1). rewrite H2; simpl.
         apply orb_false_iff; split; apply freshblock_charF; left. unfold Mem.valid_block; xomega.
         eapply Mem.valid_block_alloc; eauto. unfold Mem.valid_block; xomega.
         trivial. trivial.
       * intros bb. split; trivial. intros; apply ZZ; trivial.
     - split; trivial. split.
     { split. eapply inject_incr_trans; eauto.
       ...
       + rewrite <- (D n) in J. destruct (INCB _ _ _ J J'). split; trivial.
         ...
         rewrite <- freshblock_trans' with (m'':=m1); trivial. }
     { red; intros. specialize (D b0). destruct (eq_block b0 b1); subst.
       + clear D IHalloc_variables.
         split. eapply Mem.fresh_block_alloc; eauto.
         specialize (INCA _ _ _ C). rewrite INCA in J'. symmetry in J'; inv J'.
         exploit match_callstack_match_globalenvs. eapply MCS. intros [MG [VM [VTM [_ [_ ZZ]]]]].
           destruct (Plt_Ple_dec sp (Genv.genv_next tge)); simpl; trivial.
           inv MCS. exploit (ZZ sp). xomega. intros [_ X]; congruence.
       + rewrite <- (D n) in J; clear D. destruct (INCB _ _ _ J J') as [AA BB]; clear INCB.
         destruct (SEP _ _ _ J J') as [CC DD]; clear SEP.
         split; trivial. 
         intros N. elim CC. eapply (Mem.valid_block_alloc _ _ _ _ _ H _ N). }
Qed.
...
Theorem match_callstack_function_entry:
  ...
  /\ (*new*) globals_separate m tge f f'.
Proof.
  intros.
...
Lemma transl_step_correct:
  ...
  exists j', globals_separate m1 tge j j' /\ 
...
Proof.
  ...
  apply val_list_inject_forall_inject in H2. rewrite <- (Forall2_Zlength H2).
  ...
  constructor.
  apply forall_inject_val_list_inject; trivial.
Qed.
...
Definition match_sts:= fun cd j L1 st1 m1 L2 st2 m2 => cd=st1 /\ 
   match_ST L1 L2 (Genv.genv_next tge) j st1 m1 st2 m2 /\
...
+ intros. destruct MatchMu as [? (*[h*) [MS [VAL [WD MRR]](*]*)]]; subst.
  ...
  - split; auto. econstructor; eauto. (* destruct INC as [INC SEP].*)
    exploit match_callstack_match_globalenvs; eauto. intros [MG [A1 [A2 [_ [_ A5]]]]].
    eapply match_callstack_incr_bound; try eassumption.
    * eapply match_callstack_external_call_ext; try eassumption.
      ...
      { intros. case_eq (j b); intros.
        ...
        + case_eq (j' b); trivial. destruct p. intros.
          destruct (GSep _ _ _ H0 H1). elim H2; clear H2. unfold Mem.valid_block; xomega. }
      { intros. destruct (GSep _ _ _ H H0) as [AA BB]; clear GSep UnchSrc. xomega. } (* elim AA; clear AA. xomega.
      { intros. case_eq (j b1); intros.
        + destruct p. congruence. 
        + destruct (GSep _ _ _ H1 H) as [AA BB]; clear GSep UnchSrc. elim AA; clear AA. xomega. }*)
      xomega. xomega.
...
Admitted. (*the only admits are in the initial_states_match clause, and the genv-related clause. But
I'm not sure the instatiation (h:=Genv.genv_next tge is correct*)
...
End TRANSLATION. (*CompCert 2.7.1 has 2233 lines, and 
  ...
  Cminorgenproof_meminjs has 2435 lines,
  Cminorgenproof_backeffects has 2576 lines, so tweaking match_env, 
  match_callstack_external_call, adding the top-level clauses to corediagram and 
  moving to SIMs hast cost approx 400 lines (although macth_callstack_builtin is in fact not used*)
(*
Definition match_sts:= fun cd j L1 st1 m1 L2 st2 m2 => cd=st1 /\ 
   exists h, match_ST L1 L2 h j st1 m1 st2 m2 /\
   meminj_valid j L1 m1 L2 m2 /\ (forall b1 b2 d, j b1 = Some (b2, d) -> L1 b1 = L2 b2) /\
   mem_respects_readonly ge m1 /\ mem_respects_readonly tge m2.
...
+ intros. destruct MS as [? [h [MS [VAL [WD MRR]]]]]; eauto.
+ admit. (*genv properties - maybe these clauses are not quite right in mini_simulations.v ...*)
+ intros. destruct v; inv H. simpl. destruct (Int.eq_dec i Int.zero); inv H7.
  remember (Genv.find_funct_ptr ge b) as d; destruct d; inv H6. symmetry in Heqd. destruct f; inv H7. 
  exploit function_ptr_translated; eauto. intros [tf [FIND TR]]. 
  exploit bind_inversion. apply TR. intros [? [? ?]]. inv H7. rewrite FIND.
  ...
  eexists.
  split. reflexivity.
  split; trivial. exists (Pos.max (Genv.genv_next ge) (Genv.genv_next tge)). split. split. Search Pos.max. split. apply Pos.le_max_l. apply Pos.le_max_r.
  admit.
  intuition.
  red; intros. intuition. eapply Mem.valid_block_inject_1; eauto. eapply Mem.valid_block_inject_2; eauto.
+ intros. destruct H as [? [h [MS [VAL [WD MRR]]]]]; subst.
  destruct c1; inv H0. destruct k; inv H1.
  destruct MS as [GE MS]. inv MS. inv MK. exists tv; intuition.
+ intros. destruct H as [? [h [MS [VAL [WD MRR]]]]]; subst.
