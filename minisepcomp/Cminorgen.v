...
(** The main task in translating Csharpminor to Cminor is to explicitly
  ...
  into the simpler, lower-level [switch] constructs of Cminor_memsem.
*)
...
Definition transl_constant (cst: Csharpminor_memsem.constant): constant :=
  match cst with
  | Csharpminor_memsem.Ointconst n =>
      Ointconst n
  | Csharpminor_memsem.Ofloatconst n =>
      Ofloatconst n
  | Csharpminor_memsem.Osingleconst n =>
      Osingleconst n
  | Csharpminor_memsem.Olongconst n =>
      Olongconst n
  end.

(** Translation of expressions.  *)

Fixpoint transl_expr (cenv: compilenv) (e: Csharpminor_memsem.expr)
                     {struct e}: res expr :=
  match e with
  | Csharpminor_memsem.Evar id =>
      OK (Evar id)
  | Csharpminor_memsem.Eaddrof id =>
      OK (var_addr cenv id)
  | Csharpminor_memsem.Econst cst =>
      OK (Econst (transl_constant cst))
  | Csharpminor_memsem.Eunop op e1 =>
      do te1  <- transl_expr cenv e1;
      OK (Eunop op te1)
  | Csharpminor_memsem.Ebinop op e1 e2 =>
      do te1 <- transl_expr cenv e1;
      do te2 <- transl_expr cenv e2;
      OK (Ebinop op te1 te2)
  | Csharpminor_memsem.Eload chunk e =>
      do te <- transl_expr cenv e;
      OK (Eload chunk te)
  end.

Fixpoint transl_exprlist (cenv: compilenv) (el: list Csharpminor_memsem.expr)
...
Fixpoint transl_stmt (cenv: compilenv) (xenv: exit_env) (s: Csharpminor_memsem.stmt)
                     {struct s}: res stmt :=
  match s with
  | Csharpminor_memsem.Sskip =>
      OK Sskip
  | Csharpminor_memsem.Sset id e =>
      do te <- transl_expr cenv e;
      OK (Sassign id te)
  | Csharpminor_memsem.Sstore chunk e1 e2 =>
      do te1 <- transl_expr cenv e1;
      do te2 <- transl_expr cenv e2;
      OK (Sstore chunk te1 te2)
  | Csharpminor_memsem.Scall optid sig e el =>
      do te <- transl_expr cenv e;
      do tel <- transl_exprlist cenv el;
      OK (Scall optid sig te tel)
  | Csharpminor_memsem.Sbuiltin optid ef el =>
      do tel <- transl_exprlist cenv el;
      OK (Sbuiltin optid ef tel)
  | Csharpminor_memsem.Sseq s1 s2 =>
      do ts1 <- transl_stmt cenv xenv s1;
      do ts2 <- transl_stmt cenv xenv s2;
      OK (Sseq ts1 ts2)
  | Csharpminor_memsem.Sifthenelse e s1 s2 =>
      do te <- transl_expr cenv e;
      do ts1 <- transl_stmt cenv xenv s1;
      do ts2 <- transl_stmt cenv xenv s2;
      OK (Sifthenelse te ts1 ts2)
  | Csharpminor_memsem.Sloop s =>
      do ts <- transl_stmt cenv xenv s;
      OK (Sloop ts)
  | Csharpminor_memsem.Sblock s =>
      do ts <- transl_stmt cenv (true :: xenv) s;
      OK (Sblock ts)
  | Csharpminor_memsem.Sexit n =>
      OK (Sexit (shift_exit xenv n))
  | Csharpminor_memsem.Sswitch long e ls =>
      let (tbl, dfl) := switch_table ls O in
      do te <- transl_expr cenv e;
      transl_lblstmt cenv (switch_env ls xenv) ls (Sswitch long te tbl dfl)
  | Csharpminor_memsem.Sreturn None =>
      OK (Sreturn None)
  | Csharpminor_memsem.Sreturn (Some e) =>
      do te <- transl_expr cenv e;
      OK (Sreturn (Some te))
  | Csharpminor_memsem.Slabel lbl s =>
      do ts <- transl_stmt cenv xenv s; OK (Slabel lbl ts)
  | Csharpminor_memsem.Sgoto lbl =>
      OK (Sgoto lbl)
  end

with transl_lblstmt (cenv: compilenv) (xenv: exit_env) (ls: Csharpminor_memsem.lbl_stmt) (body: stmt)
                    {struct ls}: res stmt :=
  match ls with
  | Csharpminor_memsem.LSnil =>
      OK (Sseq (Sblock body) Sskip)
  | Csharpminor_memsem.LScons _ s ls' =>
      do ts <- transl_stmt cenv xenv s;
...
Definition build_compilenv (f: Csharpminor_memsem.function) : compilenv * Z :=
  assign_variables (PTree.empty Z, 0) (VarSort.sort (Csharpminor_memsem.fn_vars f)).
...
Definition transl_funbody
      (cenv: compilenv) (stacksize: Z) (f: Csharpminor_memsem.function): res function :=
   do tbody <- transl_stmt cenv nil f.(Csharpminor_memsem.fn_body);
       OK (mkfunction
              (Csharpminor_memsem.fn_sig f)
              (Csharpminor_memsem.fn_params f)
              (Csharpminor_memsem.fn_temps f)
              stacksize
              tbody).

Definition transl_function (f: Csharpminor_memsem.function): res function :=
...
Definition transl_fundef (f: Csharpminor_memsem.fundef): res fundef :=
  transf_partial_fundef transl_function f.

Definition transl_program (p: Csharpminor_memsem.program) : res program :=
