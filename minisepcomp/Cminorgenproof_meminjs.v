...
Lemma transl_step_correct:
  ...
  forall T1 tm1 L TL h j, match_states L TL h j S1 m1 T1 tm1 ->
  exists j',
...
Proof.
  induction 1; intros T1 tm1 L TL h j MSTATE; inv MSTATE; try rewrite freshblock_irrefl', orb_false_r_ext.

- (* skip seq *)
  monadInv TR. exists j; left.
...
- (* set *)
  ...
  exists j; left; eexists; econstructor; split.
...
- (* builtin *)
  ...
  exists f'; left; eexists; econstructor; split.
  eexists; apply effstep_plus_one. econstructor; eauto.
...
- (* seq *)
  ...
  econstructor; eauto.
- (* seq 2 *)
  exists j; right. split. auto. econstructor; eauto.

- (* ifthenelse *)
  monadInv TR.
  exploit transl_expr_correct; eauto. intros [tv [EVAL VINJ]].
  exists j; left; exists (State tfn (if b then x0 else x1) tk (Vptr sp Int.zero) te); eexists; split.
...
- (* switch *)
  ...
  exists j; left; exists T2, tm1; split.
...
- (* label *)
  monadInv TR.
  exists j; left; econstructor; eexists; split.
...
- (* internal call *)
  ...
  exists f2; left; econstructor; eexists; split.
...
- (* external call *)
  ...
  eapply inj_preserves_globals; eauto.
  intros [f' [vres' [tm' [EC [VINJ [MINJ' [UNMAPPED [OUTOFREACH [INCR SEPARATED]]]]]]]]].
  exists f'; left; econstructor; eexists; split.
...
End TRANSLATION. (*CompCert 2.7.1 has 2233 lines, and 
  ...
  Cminorgenproof_effects has 2433 lines so exposing meminjs in match_states has added 2 lines*)

(* /\
                  (forall 
            (UHyp: forall b1 z, E b1 z = true -> (L b1 = true \/ j b1 <> None))
            b2 ofs (Ub: TE b2 ofs = true),
            TL b2 = true \/ (exists b1 delta, j b1 = Some(b2, delta) /\ E b1 (ofs-delta) = true)))*)
