...
Add LoadPath "../ccc26x86" as ccc26x86.
...
Module AxiomSem.
  ...
  Record threadwise_exec :=
  { Acts : list actid            ; (* List of instances *)
    lab  : actid -> option act    ; (* Mapping of instances to actions *)
    po   : actid -> actid -> Prop       (* Program order *)
  }.

  Definition empty : threadwise_exec :=
    {| Acts := nil;
       lab := fun x => None;
       po := fun x y => False
    |}.
  
  Definition singl a (label : act) : threadwise_exec :=
    {| Acts := a :: nil;
      lab := fun x => if actid_eq x a then Some label else None;
      po := fun x y => False
    |}.

  Definition add (a:actid*act) (o: threadwise_exec) : threadwise_exec :=
    {| Acts := (fst a) :: (Acts o);
       lab := fun x => if actid_eq x (fst a) then Some (snd a) else (lab o) x;
       po := fun x y => (x = (fst a) /\ In y (Acts o)) \/ (po o) x y
    |}.
                                                               
  Definition union (o1 o2 : threadwise_exec) : threadwise_exec :=
    {| Acts := (Acts o1) ++ (Acts o2);
       lab := fun x => match (lab o1) x with
                    | Some l => Some l
                    | None => (lab o2) x
                    end;
       po := fun x y => (po o1 x y) \/ (po o2 x y) \/ (In x (Acts o1) /\ In y (Acts o2))
    |}.

    Notation " g1 ;; g2" := (union g1 g2) (at level 100).
...
Inductive load (chunk: memory_chunk) (b: block) (ofs: Z) (mv: list memval): threadwise_exec -> Prop :=
| LoadVal:
    forall (e : actid)
      (Hvalid_access: (align_chunk chunk | ofs)%Z)
      (Hsize: size_chunk_nat chunk = length mv),
      load chunk b ofs mv
           (singl e (Read b ofs chunk mv)).

Inductive store (chunk: memory_chunk) (b: block) (ofs: Z) (v:val)
  : threadwise_exec -> Prop :=
| StoreVal:
    forall (e : actid)
      (Hvalid_access: (align_chunk chunk | ofs)%Z),
      store chunk b ofs v
            (singl e (Write b ofs chunk (encode_val chunk v))).

Inductive load_nf (chunk: memory_chunk) (a: addrmode)
          (rs: regset) (rd: preg) : regset -> threadwise_exec -> Prop :=
| LoadNF:
    forall (e : actid) b ofs mv o
...
(destroyed: list preg) : regset -> threadwise_exec -> Prop :=
| StoreNF:
    forall (e : actid) b ofs o
      (Haddr: eval_addrmode ge a rs = Vptr b ofs)
      (Hstore: store chunk b (Int.unsigned ofs) (rs r1) o),
      store_nf chunk a rs r1 destroyed (nextinstr_nf (undef_regs destroyed rs)) o.

Inductive alloc (ofs : Z) (sz : Z) : threadwise_exec -> block -> Prop :=
| AllocSuccess:
    forall e b,
      alloc ofs sz (singl e (Alloc b ofs sz)) b.


Inductive step_instr (f: function) (rs: regset) : instruction ->
  regset -> threadwise_exec -> Prop :=
(** Moves *)
| Mov_rr:
    forall rd r1,
      step_instr f rs (Pmov_rr rd r1) (nextinstr (rs#rd <- (rs r1))) empty 
| Mov_ri:
    forall rd n,
      step_instr f rs (Pmov_ri rd n) (nextinstr_nf (rs#rd <- (Vint n))) empty
| Mov_ra:
    forall rd id,
      step_instr f rs (Pmov_ra rd id)
                 (nextinstr_nf (rs#rd <- (Genv.symbol_address ge id Int.zero))) empty
| Mov_rm:
    forall (rd : ireg) a rs' o
      (Hload: load_nf Mint32 a rs rd rs' o),
      step_instr f rs (Pmov_rm rd a) rs' o
| Mov_mr:
    forall a (r1:ireg) rs' o
      (Hstore: store_nf Mint32 a rs r1 nil rs' o),
      step_instr f rs (Pmov_mr a r1) rs' o
| Movsd_ff:
    forall rd r1,
      step_instr f rs (Pmovsd_ff rd r1) (nextinstr (rs#rd <- (rs r1))) empty
| Movsd_fi:
    forall rd n,
      step_instr f rs (Pmovsd_fi rd n) (nextinstr (rs#rd <- (Vfloat n))) empty
| Movsd_fm:
      forall (rd:freg) a rs' o
      (Hload: load_nf Mfloat64 a rs rd rs' o),
      step_instr f rs (Pmovsd_fm rd a) rs' o
| Movsd_mf:
    forall a (r1:freg) rs' o
      (Hstore: store_nf Mfloat64 a rs r1 nil rs' o),
      step_instr f rs (Pmovsd_mf a r1) rs' o
| Movss_fi:
    forall rd n,
      step_instr f rs (Pmovss_fi rd n) (nextinstr (rs#rd <- (Vsingle n))) empty
| Movss_fm:
    forall (rd:freg) a rs' o
      (Hload: load_nf Mfloat32 a rs rd rs' o),
      step_instr f rs (Pmovss_fm rd a) rs' o
| Movss_mf:
    forall a (r1:freg) rs' o
      (Hstore: store_nf Mfloat32 a rs r1 nil rs' o),
      step_instr f rs (Pmovss_mf a r1) rs' o
| Fldl_m:
  forall a rs' o
     (Hload: load_nf Mfloat64 a rs ST0 rs' o),
    step_instr f rs (Pfldl_m a) rs' o
| Fstpl_m:
    forall a rs' o
      (Hstore: store_nf Mfloat64 a rs ST0 (ST0 :: nil) rs' o),
      step_instr f rs (Pfstpl_m a) rs' o
| Flds_m:
    forall a rs' o
      (Hload: load_nf Mfloat32 a rs ST0 rs' o),
      step_instr f rs (Pflds_m a) rs' o
| Fstps_m:
    forall a rs' o
      (Hstore: store_nf Mfloat32 a rs ST0 (ST0 :: nil) rs' o),
      step_instr f rs (Pfstps_m a) rs' o
| Xchg_rr:
    forall r1 r2,
      step_instr f rs (Pxchg_rr r1 r2)
                 (nextinstr (rs#r1 <- (rs r2) #r2 <- (rs r1))) empty
(** Moves with conversion *)
| Movb_mr:
    forall a (r1:ireg) rs' o
      (Hstore: store_nf Mint8unsigned a rs r1 nil rs' o),
      step_instr f rs (Pmovb_mr a r1) rs' o
| Movw_mr:
    forall a (r1:ireg) rs' o
      (Hstore: store_nf Mint16unsigned a rs r1 nil rs' o),
      step_instr f rs (Pmovw_mr a r1) rs' o
| Movzb_rr:
    forall rd r1,
      step_instr f rs (Pmovzb_rr rd r1)
                 (nextinstr (rs#rd <- (Val.zero_ext 8 rs#r1))) empty
| Movzb_rm:
    forall (rd:ireg) a rs' o
      (Hload: load_nf Mint8unsigned a rs rd rs' o),
      step_instr f rs (Pmovzb_rm rd a) rs' o
| Movsb_rr:
    forall rd r1,
      step_instr f rs (Pmovsb_rr rd r1)
                 (nextinstr (rs#rd <- (Val.sign_ext 8 rs#r1))) empty
| Movsb_rm:
    forall (rd:ireg) a rs' o
      (Hload: load_nf Mint8signed a rs rd rs' o),
      step_instr f rs (Pmovsb_rm rd a) rs' o
| Movzw_rr:
    forall rd r1,
      step_instr f rs (Pmovzw_rr rd r1)
                 (nextinstr (rs#rd <- (Val.zero_ext 16 rs#r1))) empty
| Movzw_rm:
    forall (rd:ireg) a rs' o
      (Hload: load_nf Mint16unsigned a rs rd rs' o),
      step_instr f rs (Pmovzw_rm rd a) rs' o
| Movsw_rr:
    forall rd r1,
      step_instr f rs (Pmovsw_rr rd r1)
                  (nextinstr (rs#rd <- (Val.sign_ext 16 rs#r1))) empty
| Movsw_rm:
    forall (rd:ireg) a rs' o
      (Hload: load_nf Mint16signed a rs rd rs' o),
      step_instr f rs (Pmovsw_rm rd a) rs' o
| Cvtsd2ss_ff:
    forall rd r1,
      step_instr f rs (Pcvtsd2ss_ff rd r1)
                 (nextinstr (rs#rd <- (Val.singleoffloat rs#r1))) empty
| Cvtss2sd_ff:
    forall rd r1,
      step_instr f rs (Pcvtsd2ss_ff rd r1)
                 (nextinstr (rs#rd <- (Val.floatofsingle rs#r1))) empty
| Cvttsd2si_rf:
    forall rd r1,
      step_instr f rs (Pcvttsd2si_rf rd r1)
                 (nextinstr (rs#rd <- (Val.maketotal (Val.intoffloat rs#r1)))) empty
| Cvtsi2sd_fr:
    forall rd r1,
      step_instr f rs (Pcvtsi2sd_fr rd r1)
                 (nextinstr (rs#rd <- (Val.maketotal (Val.floatofint rs#r1)))) empty
| Cvttss2si_rf:
    forall rd r1,
      step_instr f rs (Pcvttss2si_rf rd r1)
                 (nextinstr (rs#rd <- (Val.maketotal (Val.intofsingle rs#r1)))) empty
| Cvtsi2ss_fr:
    forall rd r1,
      step_instr f rs (Pcvtsi2ss_fr rd r1)
                 (nextinstr (rs#rd <- (Val.maketotal (Val.singleofint rs#r1)))) empty
(** Integer arithmetic *)
| Lea:
    forall rd a,
      step_instr f rs (Plea rd a)
                 (nextinstr (rs#rd <- (eval_addrmode ge a rs))) empty
| Neg:
    forall rd,
      step_instr f rs (Pneg rd)
                 (nextinstr_nf (rs#rd <- (Val.neg rs#rd))) empty
| Sub_rr:
    forall rd r1,
      step_instr f rs (Psub_rr rd r1)
                 (nextinstr_nf (rs#rd <- (Val.sub rs#rd rs#r1))) empty
| Imul_rr:
    forall rd r1,
      step_instr f rs (Pimul_rr rd r1)
                 (nextinstr_nf (rs#rd <- (Val.mul rs#rd rs#r1))) empty
| Imul_ri:
    forall rd n,
      step_instr f rs (Pimul_ri rd n)
                 (nextinstr_nf (rs#rd <- (Val.mul rs#rd (Vint n)))) empty
| Imul_r:
    forall r1,
      step_instr f rs (Pimul_r r1)
                 (nextinstr_nf (rs#EAX <- (Val.mul rs#EAX rs#r1)
                                        #EDX <- (Val.mulhs rs#EAX rs#r1))) empty
| Mul_r:
    forall r1,
      step_instr f rs (Pmul_r r1)
                 (nextinstr_nf (rs#EAX <- (Val.mul rs#EAX rs#r1)
                                        #EDX <- (Val.mulhu rs#EAX rs#r1))) empty
| Div:
    forall (r1:ireg) vq vr,
      ...
      step_instr f rs (Pdiv r1) (nextinstr_nf (rs#EAX <- vq #EDX <- vr)) empty
| And_rr:
    forall rd r1,
      step_instr f rs (Pand_rr rd r1) (nextinstr_nf (rs#rd <- (Val.and rs#rd rs#r1))) empty
| And_ri:
    forall rd n,
      step_instr f rs (Pand_ri rd n) (nextinstr_nf (rs#rd <- (Val.and rs#rd (Vint n)))) empty
| Or_rr:
    forall rd r1,
      step_instr f rs (Por_rr rd r1) (nextinstr_nf (rs#rd <- (Val.or rs#rd rs#r1))) empty
| Or_ri:
    forall rd n,
      step_instr f rs (Por_ri rd n) (nextinstr_nf (rs#rd <- (Val.or rs#rd (Vint n)))) empty
| Xor_r:
    forall rd,
      step_instr f rs (Pxor_r rd) (nextinstr_nf (rs#rd <- Vzero)) empty
| Xor_rr:
    forall rd r1,
      step_instr f rs (Pxor_rr rd r1) (nextinstr_nf (rs#rd <- (Val.xor rs#rd rs#r1))) empty
| Xor_ri:
    forall rd n,
      step_instr f rs (Pxor_ri rd n) (nextinstr_nf (rs#rd <- (Val.xor rs#rd (Vint n)))) empty
| Not:
    forall rd,
      step_instr f rs (Pnot rd) (nextinstr_nf (rs#rd <- (Val.notint rs#rd))) empty
| Sal_rcl:
    forall rd,
      step_instr f rs (Psal_rcl rd) (nextinstr_nf (rs#rd <- (Val.shl rs#rd rs#ECX))) empty
| Sal_ri:
    forall rd n,
      step_instr f rs (Psal_ri rd n) (nextinstr_nf (rs#rd <- (Val.shl rs#rd (Vint n)))) empty
| Shr_rcl:
    forall rd,
      step_instr f rs (Pshr_rcl rd) (nextinstr_nf (rs#rd <- (Val.shru rs#rd rs#ECX))) empty
| Shr_ri:
    forall rd n,
      step_instr f rs (Pshr_ri rd n) (nextinstr_nf (rs#rd <- (Val.shru rs#rd (Vint n)))) empty
| Sar_rcl:
  forall rd,
    step_instr f rs (Psar_rcl rd) (nextinstr_nf (rs#rd <- (Val.shr rs#rd rs#ECX))) empty
| Sar_ri:
    forall rd n,
      step_instr f rs (Psar_ri rd n) (nextinstr_nf (rs#rd <- (Val.shr rs#rd (Vint n)))) empty
| Shld_ri:
    forall rd r1 n,
...
| Ror_ri:
    forall rd n,
      step_instr f rs (Pror_ri rd n) (nextinstr_nf (rs#rd <- (Val.ror rs#rd (Vint n)))) empty
| Cmp_rr:
    forall r1 r2,
      step_instr f rs (Pcmp_rr r1 r2) (nextinstr (compare_ints (rs r1) (rs r2) rs)) empty
| Cmp_ri:
    forall r1 n,
      step_instr f rs (Pcmp_ri r1 n) (nextinstr (compare_ints (rs r1) (Vint n) rs)) empty
| Test_rr:
    forall r1 r2,
      step_instr f rs (Ptest_rr r1 r2) (nextinstr (compare_ints (Val.and (rs r1) (rs r2)) Vzero rs)) empty
| Test_ri:
    forall r1 n,
      step_instr f rs (Ptest_ri r1 n) (nextinstr (compare_ints (Val.and (rs r1) (Vint n)) Vzero rs)) empty
| Cmov:
    forall c rd r1,
...
| Setcc:
    forall c rd,
      step_instr f rs (Psetcc c rd) (nextinstr (rs#rd <- (Val.of_optbool (eval_testcond c rs)))) empty
(** Arithmetic operations over double-precision floats *)
| Addd_ff:
    forall rd r1,
      step_instr f rs (Paddd_ff rd r1) (nextinstr (rs#rd <- (Val.addf rs#rd rs#r1))) empty
| Subd_ff:
    forall rd r1,
      step_instr f rs (Psubd_ff rd r1) (nextinstr (rs#rd <- (Val.subf rs#rd rs#r1))) empty
| Muld_ff:
    forall rd r1,
      step_instr f rs (Pmuld_ff rd r1)  (nextinstr (rs#rd <- (Val.mulf rs#rd rs#r1))) empty
| Divd_ff:
    forall rd r1,
      step_instr f rs (Pdivd_ff rd r1) (nextinstr (rs#rd <- (Val.divf rs#rd rs#r1))) empty
| Negd:
    forall rd, 
      step_instr f rs (Pnegd rd) (nextinstr (rs#rd <- (Val.negf rs#rd))) empty
| Absd:
    forall rd,
      step_instr f rs (Pabsd rd) (nextinstr (rs#rd <- (Val.absf rs#rd))) empty
| Comisd_ff:
    forall r1 r2,
      step_instr f rs (Pcomisd_ff r1 r2) (nextinstr (compare_floats (rs r1) (rs r2) rs)) empty
| Xorpd_f:
    forall rd,
      step_instr f rs (Pxorpd_f rd) (nextinstr_nf (rs#rd <- (Vfloat Float.zero))) empty
(** Arithmetic operations over single-precision floats *)
| Adds_ff:
    forall rd r1,
      step_instr f rs (Padds_ff rd r1) (nextinstr (rs#rd <- (Val.addfs rs#rd rs#r1))) empty
| Subs_ff:
    forall rd r1,
      step_instr f rs (Psubs_ff rd r1) (nextinstr (rs#rd <- (Val.subfs rs#rd rs#r1))) empty
| Muls_ff:
    forall rd r1,
      step_instr f rs (Pmuls_ff rd r1) (nextinstr (rs#rd <- (Val.mulfs rs#rd rs#r1))) empty
| Divs_ff:
    forall rd r1,
      step_instr f rs (Pdivs_ff rd r1) (nextinstr (rs#rd <- (Val.divfs rs#rd rs#r1))) empty
| Negs:
    forall rd,
      step_instr f rs (Pnegs rd) (nextinstr (rs#rd <- (Val.negfs rs#rd))) empty
| Abss:
    forall rd,
      step_instr f rs (Pabss rd) (nextinstr (rs#rd <- (Val.absfs rs#rd))) empty
| Comiss_ff:
    forall r1 r2,
      step_instr f rs (Pcomiss_ff r1 r2) (nextinstr (compare_floats32 (rs r1) (rs r2) rs)) empty
| Xorps_f:
    forall rd,
      step_instr f rs (Pxorps_f rd) (nextinstr_nf (rs#rd <- (Vsingle Float32.zero))) empty
  (** Branches and calls *)
| Jmp_l:
  forall lbl rs'
    (Hgoto: goto_label f lbl rs rs'),
    step_instr f rs (Pjmp_l lbl) rs' empty
| Jmp_s:
    forall id sg,
      step_instr f rs (Pjmp_s id sg) (rs#PC <- (Genv.symbol_address ge id Int.zero)) empty
| Jmp_r:
    forall r sg,
      step_instr f rs (Pjmp_r r sg)  (rs#PC <- (rs r)) empty
| Jcc:
    forall cond lbl rs'
      ...
      step_instr f rs (Pjcc cond lbl) rs' empty
| Jcc2:
    forall cond1 cond2 lbl rs'
      ...
      step_instr f rs (Pjcc2 cond1 cond2 lbl) rs' empty
| Jmptbl:
    forall (r:ireg) tbl n lbl rs'
      (Hrs: rs#r = Vint n)
      (Htbl: list_nth_z tbl (Int.unsigned n) = Some lbl)
      (Hgoto: goto_label f lbl rs rs'),
      step_instr f rs (Pjmptbl r tbl) rs' empty
| Call_s:
    forall id sg,
      step_instr f rs (Pcall_s id sg) 
                 (rs#RA <- (Val.add rs#PC Vone) #PC <- (Genv.symbol_address ge id Int.zero)) empty
| Call_r:
    forall r sg,
      step_instr f rs (Pcall_r r sg) (rs#RA <- (Val.add rs#PC Vone) #PC <- (rs r)) empty
| Ret:
      step_instr f rs Pret (rs#PC <- (rs#RA)) empty
  (** Saving and restoring registers *)
| Mov_rm_a:
    forall (rd:ireg) a rs' o
      (Hload: load_nf Many32 a rs rd rs' o),
      step_instr f rs (Pmov_rm_a rd a) rs' o
| Mov_mr_a:
    forall a (r1:ireg) rs' o
      (Hstore: store_nf Many32 a rs r1 nil rs' o),
      step_instr f rs (Pmov_mr_a a r1) rs' o
| Movsd_fm_a:
    forall (rd:freg) a rs' o
      (Hload: load_nf Many64 a rs rd rs' o),
      step_instr f rs (Pmovsd_fm_a rd a) rs' o
| Movsd_mf_a:
    forall a (r1:freg) rs' o
      (Hstore: store_nf Many64 a rs r1 nil rs' o),
      step_instr f rs (Pmovsd_mf_a a r1) rs' o
  (** Pseudo-instructions *)
| Label:
    forall lbl,
      step_instr f rs (Plabel lbl) (nextinstr rs) empty
| Allocframe:
