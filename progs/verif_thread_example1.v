...
Proof.
  ...
  {
    renormalize.
    Lemma lift_sepcon A B : `A * `B = `(A * B). Proof. apply pred_ext; entailer. Qed.
    rewrite lift_sepcon.
    constructor.
    constructor.
  }
  (* specify the frame evar generated by forward_call to be [ab->lock |-> l_] *)
  ...
  (* First, we need to get the expression that evaluates _f to
  compute, so we introduce a command: pf = &f; for debugging purposes
  *)
  
  (* from verif_message, which does not compile right now *)
  Ltac get_global_function id := eapply (call_lemmas.semax_fun_id'
  id); [ reflexivity | simpl; reflexivity | ]. Lemma slide_func_ptr:
  forall f e R1 R, SEPx (`(func_ptr' f) e :: R1 :: R) = SEPx
  ((`(func_ptr f) e && R1)::R).  Proof.  intros. unfold SEPx;
  unfold_lift.  extensionality rho.  simpl.  rewrite <- sepcon_assoc.
  f_equal.  unfold func_ptr'.  rewrite corable_andp_sepcon1 by apply
  corable_func_ptr.  rewrite emp_sepcon; auto.  Qed.  Ltac
  get_global_function' id := eapply (call_lemmas.semax_fun_id' id); [
  reflexivity | reflexivity | simpl; reflexivity | rewrite
  slide_func_ptr ].
  
  (* for some reason the tactic needs "_ && PROP ..." *)
  match goal with [ |- context [ PROPx nil ?a ] ] =>
  replace  (PROPx [] a) with (!!True && PROPx [] a) by (apply pred_ext; entailer)
  end.
  
  (* get_global_function _f. (* DOES NOT WORK "wrong number of tactics" *) *)
