...
Proof. inv Hinv.
  ...
  - clear. intros. destruct X. eexists; try reflexivity. 
    admit. (*mixup between languages?*)
  - clear. intros. destruct X. 2: eexists; try reflexivity. 
...
Lemma thread_step_diagram:
    ...
    thread_step (new_DMachineSem U0 r) gt U st1 m st1' m' ->
...
+ apply TEvstep. simpl. unfold ThreadPool.SEM.Sem in Hcorestep.
...
Lemma machine_step_diagram:
    ...
    machine_step (new_DMachineSem U0 r) gt U tr st1 m U' tr' st1' m' ->
...
+ inv Htstep; simpl in *.
  ...
  - apply HschedN.
  - apply mem_compatible_preserved; trivial.
  - red; simpl. apply ResumeThread with (c:=TState _ _ c)(c':=TState _ _ c')(X:=X). 
...
+ inv Htstep; simpl in *.
  ...
  -  apply mem_compatible_preserved; trivial.
  - red; simpl. apply SuspendThread with (c:=TState _ _ c)(X:=X). 
...
+ inv Htstep; simpl in *.
  { (*acquire*)
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_acquire hb Sems Semt true (gs,gt))with (c:=TState _ _ c)(Hlt':=Hlt'); simpl.
      * apply Hbounded.
      * apply invariant_preserved; trivial.
      * unfold ThreadPool.getThreadC in Hcode. destruct st1; simpl in *. rewrite Hcode; trivial.
      * apply Hat_external.
  ...
  { (*release*) 
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_release hb Sems Semt true (gs,gt))with (c:=TState _ _ c)(Hlt':=Hlt'); simpl.
    * apply Hbounded.
    * apply HboundedLP. 
    * apply invariant_preserved; trivial.
    * unfold ThreadPool.getThreadC in Hcode. destruct st1; simpl in *. rewrite Hcode; trivial.
  ...
  { (*spawn*) 
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_create hb Sems Semt true (gs,gt))with (c:=TState _ _ c); simpl.
    * apply Hbounded.
    * apply Hbounded_new. 
    * apply invariant_preserved; trivial.
    * unfold ThreadPool.getThreadC in Hcode. destruct st1; simpl in *. rewrite Hcode; trivial.
  ...
  { (*makelock*) 
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_mklock hb Sems Semt true (gs,gt))with (c:=TState _ _ c)(pmap_tid':=pmap_tid'); simpl.
    * apply invariant_preserved; trivial.
    * unfold ThreadPool.getThreadC in Hcode. destruct st1; simpl in *. rewrite Hcode; trivial.
    * apply Hat_external.
  ...
  { (*freelock*) 
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_freelock hb Sems Semt true (gs,gt))with (c:=TState _ _ c)(pmap_tid':=pmap_tid')(rmap:=rmap); simpl.
    * apply Hbounded.
    * apply invariant_preserved; trivial.
    * unfold ThreadPool.getThreadC in Hcode. destruct st1; simpl in *. rewrite Hcode; trivial.
    * apply Hat_external.
  ...
  { (*acquire_fail*) 
    ...
    - apply HschedN.
    - reflexivity. (*double goal seems odd*)
    - red; simpl. eapply (@HybridMachine.step_acqfail hb Sems Semt true (gs,gt))with (c:=TState _ _ c); simpl.
    * apply invariant_preserved; trivial.
    * unfold ThreadPool.getThreadC in Hcode. destruct st1'; simpl in *. rewrite Hcode; trivial.
    * apply Hat_external.
    * instantiate (1:=(permissions.restrPermMap (proj2 (Hcmpt tid Htid)))). destruct st1'; simpl. f_equal. f_equal. f_equal. simpl. 
...
+ (*halted*)
  ...
  - apply mem_compatible_preserved; trivial.
  - apply invariant_preserved; trivial.
  - red; simpl. apply (@HybridMachine.thread_halted' hb Sems Semt) with (c:=TState _ _ c); simpl. 
    * unfold ThreadPool.getThreadC in Hcode. destruct st1'; simpl in *. rewrite Hcode; trivial.
    * apply Hcant.
+ (* schedfail *) 
  simpl in *.
