...
Module AxiomaticIntermediate.
  ...
  Section AxiomaticIntermediate.
    ...
    Variable FullE : events.
    Variable finEx: Finite _ FullE.

    (** Simulation Relation between [tp1, Ex1] and [tp2, Ex2] indexed by a nat n (number of steps)
        and a set of events Ex. *)
    Record sim (n:nat) (tp1 : t) (Ex1 : events) (tp2 : t) (Ex2 : events) :=
      { set_dis  : Disjoint _ Ex1 Ex2; (** Ex1 and Ex2 are [Disjoint] *)
        set_inv  : FullE <--> Union _ Ex1 Ex2; (** Ex is the [Union] of Ex1 and Ex2 *)
        ...
        sc_tot   : strict_total_order sc FullE;
    ...
    Proof.
      ...
      { clear - Hspawn.
        ...
        destruct HIn as [HIn | HIn];
          [now inv HIn | exfalso; now eapply concLabelsofE_no_spawn in HIn].
      }
      subst.
      split;
    ...
    Proof.
      ...
      - simpl.
        ...
        { pose proof (proj2 Henum e a ltac:(simpl; now auto)) as Hipo.
          ...
          destruct (po_trans_codom _ _ _ HpoWF n HRa2 HR12); auto.
          exfalso.
          eapply Himm;
            now eauto.
        }
    ...
    Lemma step_po_sim:
      forall n tp1 Ex1 tp2 Ex2 tp1' Ex1'
        (Hsim: sim n tp1 Ex1 tp2 Ex2)
        (Hstep_po: [tp1, Ex1] ==>po [tp1', Ex1']),
      exists Ex2',
        sim (S n) tp1' Ex1' tp2 Ex2'.
    Proof.
      intros.
      inversion Hstep_po.
      destruct Hsim; subst.
      ...
      - (** Ex2 is the history of the execution: events in Ex1 cannot be po-before events in Ex2*)
        ...
        + (** If e2 is in es *)
          ...
          * (** If e2 <> e' then po e' e2 by the enumeration of es *)
            ...
            (*NOTE: i suspect we need this in the simulation as well *)
            (* assert (Ex2_closed: forall a b, po a b /\ b \in Ex2 -> a \in Ex2). *)
            destruct (po_trans_codom _ _ _ po_wf0 Hneq Hcontra Hpo2) as [Hpo1' | Hpo1'].
            ** eapply Hmin.
               eexists;
                 now eauto.
...
(** * Part 2 *)
Module IntermediateOperational.
  ...
  Section IntermediateOperational.
    Context
      {lbl : Labels}
      {sem : Semantics}
      {threadpool : ThreadPool C}
