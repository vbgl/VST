...
Require Import digests.
Require Import digest.
Instance CompSpecs : compspecs. make_compspecs prog. Defined.
Definition Vprog : varspecs. mk_varspecs prog. Defined.

Require Import digest_model.
Eval compute in reptype (Tstruct _env_md_st noattr). 
(*(val *  (val * (val * (val * (val * (val * (val *      val)))))))*)
(* type, mdsize, flags, init, update, final, blocksize, ctxsize*)
...
Definition EVP_MD_block_size_SPEC := DECLARE _EVP_MD_block_size
  ...
  POST [tuint]
       ...
       SEP (EVP_MD_rep R md).
Eval compute in reptype (Tstruct _env_md_ctx_st noattr).
(*val (val * (val * val))*)
(*digest mddata pctx pctxops*)

Parameter EVP_MD_data_rep: forall (C: ENV_MD_CTX) (v:val), mpred.
(*TODO: refine - should have data_at that models some EVP_MD_CTX_state
  according to the given C's EVP_MDtype and (if in state "hashed" the correct content*)

Definition matchEVP_MD_CTX (C: ENV_MD_CTX) (vals:reptype (Tstruct _env_md_ctx_st noattr)) :=
match vals with (md, (mddata, _)) => (*TODO:pctx not yet modeled*)
  EVP_MD_rep (digest_of_ctx C) md *
  EVP_MD_data_rep C mddata
end.
(*
Definition EVP_MD_CTX_rep (C: ENV_MD_CTX) (p:val):mpred :=
  EX sh:_, EX vals :_,
  !!readable_share sh &&
  (matchEVP_MD_CTX C vals * data_at sh (Tstruct _env_md_ctx_st noattr) vals p).
*)
(*
Definition EVP_MD_CTX_rep' (C: ENV_MD_CTX) (d p:val):mpred :=
...
(*nonnull node*)
Definition EVP_MD_CTX_NNnode (C: ENV_MD_CTX) (vals:val*(val*(val*val))) (p:val):mpred :=
...
Lemma EVP_MD_CTX_NNnode_isptr C vals p:
  EVP_MD_CTX_NNnode C vals p |-- !!(isptr p).
Proof.
  unfold EVP_MD_CTX_NNnode. destruct vals as [d [mdd [pc pcops]]]. normalize.
  rewrite data_at_isptr. entailer!.
Qed.
Lemma EVP_MD_CTX_NNnode_isptr' C vals p:
  EVP_MD_CTX_NNnode C vals p  = (!!(isptr p) &&  EVP_MD_CTX_NNnode C vals p).
Proof.
  apply pred_ext; [ entailer; apply EVP_MD_CTX_NNnode_isptr | entailer!].
Qed.

(*a possibly null node*)
Definition EVP_MD_CTX_node (C: ENV_MD_CTX) (vals:val*(val*(val*val))) (p:val):mpred :=
 (!!(p = nullval) && emp) || EVP_MD_CTX_NNnode C vals p.
...
Definition EVP_MD_CTX_md_SPEC := DECLARE _EVP_MD_CTX_md
  WITH ctx:val, C:ENV_MD_CTX, d:val, x:val * (val * val)
  PRE [ _ctx OF (tptr (Tstruct _env_md_ctx_st noattr)) ]
      PROP ()
      LOCAL (temp _ctx ctx)
      SEP (EVP_MD_CTX_node C (d, x) ctx)
...
Definition EVP_MD_CTX_size_SPEC := DECLARE _EVP_MD_CTX_size
  ...
  PRE [ _ctx OF (tptr (Tstruct _env_md_ctx_st noattr)) ]
      PROP ()
      LOCAL (temp _ctx ctx)
      SEP (EVP_MD_CTX_NNnode C (d, x) ctx;  EVP_MD_rep (digest_of_ctx C) d)
  POST [ tptr (Tstruct _env_md_st noattr) ]
       PROP ()
       LOCAL (temp ret_temp (Vint (Int.repr (EVP_MD_rec_md_size (digest_of_ctx C)))))
       SEP (EVP_MD_CTX_NNnode C (d, x) ctx;  EVP_MD_rep (digest_of_ctx C) d).
...
Definition EVP_MD_CTX_block_size_SPEC := DECLARE _EVP_MD_CTX_block_size
  ...
  POST [ tptr (Tstruct _env_md_st noattr) ]
       PROP ()
       LOCAL (temp ret_temp (Vint (Int.repr (EVP_MD_rec_block_size (digest_of_ctx C)))))
       SEP (EVP_MD_CTX_NNnode C (d, x) ctx;  EVP_MD_rep (digest_of_ctx C) d).
...
Definition EVP_MD_CTX_type_SPEC := DECLARE _EVP_MD_CTX_type
  ...
  POST [ tptr (Tstruct _env_md_st noattr) ]
       PROP ()
       LOCAL (temp ret_temp (Vint (Int.repr (NID_of_DigestNID (EVP_MD_rec_type (digest_of_ctx C))))))
       SEP (EVP_MD_CTX_NNnode C (d, x) ctx;  EVP_MD_rep (digest_of_ctx C) d).
...
Definition EVP_DigestUpdate_SPEC := DECLARE _EVP_DigestUpdate
  WITH ctx:val, data:_, len:_, C:ENV_MD_CTX, d:val, x:val * (val * val)
  PRE [ _ctx OF (tptr (Tstruct _env_md_ctx_st noattr)), 
        _data OF (tptr tvoid), _len OF tuint ]
      PROP ()
      LOCAL (temp _ctx ctx; temp _data data; temp _len len)
      SEP (EVP_MD_CTX_NNnode C (d, x) ctx;  EVP_MD_rep (digest_of_ctx C) d)
  POST [ tint ]
       PROP ()
...
Definition EVP_sha256_do_init_SPEC := DECLARE _EVP_sha256_do_init
  ...
  POST [ tvoid ]
    PROP () LOCAL () SEP (data_at_ sh (Tstruct _env_md_st noattr) out).


Definition Gprog : funspecs :=
  (*FAILS TO TERMINATE: ltac:(with_library prog [EVP_MD_type_SPEC]).*)
  [EVP_sha256_do_init_SPEC; EVP_DigestUpdate_SPEC; EVP_MD_CTX_block_size_SPEC; EVP_MD_CTX_type_SPEC; EVP_MD_CTX_size_SPEC; EVP_MD_CTX_md_SPEC; EVP_MD_type_SPEC; EVP_MD_flags_SPEC; EVP_MD_size_SPEC; EVP_MD_block_size_SPEC].
(*Locate CompSpecs.
Lemma change_compspecs_t_struct_SHA256state_st:
  @data_at spec_sha.CompSpecs Tsh (Tstruct _env_md_st noattr) =
  @data_at CompSpecs (Tstruct _env_md_st noattr).
Proof.
...
Proof.
  ...
  rewrite (func_ptr'_isptr' fin). normalize. 
  forward. simpl.
  forward.
  forward. (*
  assert (R1: Tstruct digests._env_md_st = Tstruct _env_md_st). reflexivity.
  rewrite R1.
  assert (R2: Tstruct digests._env_md_ctx_st = Tstruct _env_md_ctx_st). reflexivity.
  rewrite R2.
  assert (R3: digests._out = _out) by reflexivity. 
  rewrite R3.
  ...
  destruct fin; try contradiction. simpl. normalize. forward.
eapply semax_post. 2:  forward. unfold func_ptr'.
destruct ini.
...
Definition sha256Upd_TypeTree:rmaps.TypeTree := rmaps.ProdType (rmaps.ConstType ENV_MD_CTX ) (rmaps.ConstType val).
...
Lemma body_EVP_MD_CTX_block_size: semax_body Vprog Gprog f_EVP_MD_CTX_block_size EVP_MD_CTX_block_size_SPEC.
Proof.
  start_function. 
  rewrite  EVP_MD_CTX_NNnode_isptr'; Intros.
  forward_call (ctx, C, d, x).
...
Lemma body_EVP_MD_CTX_type: semax_body Vprog Gprog f_EVP_MD_CTX_type EVP_MD_CTX_type_SPEC.
Proof.
  start_function. 
  rewrite  EVP_MD_CTX_NNnode_isptr'; Intros.
  forward_call (ctx, C, d, x).
...
Lemma body_EVP_MD_CTX_size: semax_body Vprog Gprog f_EVP_MD_CTX_size EVP_MD_CTX_size_SPEC.
Proof.
  start_function. 
  rewrite  EVP_MD_CTX_NNnode_isptr'; Intros.
  forward_call (ctx, C, d, x).
...
Lemma body_EVP_MD_CTX_md: semax_body Vprog Gprog f_EVP_MD_CTX_md EVP_MD_CTX_md_SPEC.
Proof.
  start_function. destruct x as [mddata [pctx pctxops]].
  forward_if (
      PROP ()
      LOCAL (temp _ctx ctx)
      SEP (EVP_MD_CTX_NNnode C (d, (mddata, (pctx, pctxops))) ctx)). 
...
Lemma body_EVP_MD_flags: semax_body Vprog Gprog f_EVP_MD_flags EVP_MD_flags_SPEC. 
Proof.
  start_function.  
  unfold EVP_MD_rep. Intros sh vals; rename H into Rsh. 
  destruct vals as (type, (mdsize, (flags, (ini, (upd, (fin, (blocksize, ctxsize))))))); simpl in *.
...
Lemma body_EVP_MD_size: semax_body Vprog Gprog f_EVP_MD_size EVP_MD_size_SPEC. 
Proof.
  start_function.  
  unfold EVP_MD_rep. Intros sh vals; rename H into Rsh. 
  destruct vals as (type, (mdsize, (flags, (ini, (upd, (fin, (blocksize, ctxsize))))))); simpl in *.
...
Lemma body_EVP_MD_block_size: semax_body Vprog Gprog f_EVP_MD_block_size EVP_MD_block_size_SPEC. 
Proof.
  start_function.  
  unfold EVP_MD_rep. Intros sh vals; rename H into Rsh. 
  destruct vals as (type, (mdsize, (flags, (ini, (upd, (fin, (blocksize, ctxsize))))))); simpl in *.
