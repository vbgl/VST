...
(*
Definition confined mu m1 m2 m2' :=
  forall b2 z (Eb2: Mem.valid_block m2 b2),
         Mem.unchanged_on (fun b' z' => (b2,z)=(b',z')) m2 m2' \/
         locBlocksTgt mu b2 = true \/
         (exists b1 d, foreign_of mu b1 = Some(b2,d) /\ Mem.perm m1 b1 (z-d) Max Nonempty).*)
...
Definition confinedW mu (W1 W2: block -> Z -> bool) m2 m2':=
  forall b2 z,
  if W2 b2 z then forall (Eb2: extBlocksTgt mu b2 = true),
                         exists b1 d, foreign_of mu b1 = Some(b2,d) /\ W1 b1 (z-d) = true
  else Mem.unchanged_on (fun b' z' => (b2,z)=(b',z')) m2 m2'.

Definition confinedR mu (R1 R2: block -> Z -> bool):=
  forall b2 z,
  if R2 b2 z then forall (Eb2: extBlocksTgt mu b2 = true),
                         exists b1 d, foreign_of mu b1 = Some(b2,d) /\ R1 b1 (z-d) = true
  else True.
(*also used for MaxNonempty ie ptr comparison footprint*)
...
Definition extern_out_of_reach mu (m : mem) (b : block) (ofs : Z): Prop :=
  (*locBlocksTgt mu b = false /\ *)extBlocksTgt mu b = true /\
  (forall b0 delta, extern_of mu b0 = Some (b, delta) ->
                  (~ Mem.perm m b0 (ofs - delta) Max Nonempty \/
                   frgnBlocksSrc mu b0 = false)).

(*Lemma confined_unch mu (m1 m2 m2':mem) (WD:SM_wd mu) (Fwd: mem_forward m2 m2') (C:confined mu m1 m2 m2'):
      Mem.unchanged_on (extern_out_of_reach mu m1) m2 m2'.
Proof. constructor.
apply mem_forward_nextblock; eassumption.
{ intros b2 z k p [E1 E3(*[E2 E3]*)] V. destruct (C _ z V).
  + apply H; trivial.
  + destruct H. 
    - rewrite (locBlocksTgt_extBlocksTgt _ WD _ H) in E1; congruence.
    - destruct H as [b1 [d [FRG V1]]].
      destruct (E3 b1 d); [ apply foreign_in_extern; trivial | contradiction |]. 
      apply frgnBlocksSrc_false_foreign_None in H. congruence. }
{ intros b2 z [E1 E3(*[E2 E3]*)] P.
  exploit Mem.perm_valid_block; eauto. intros V. 
  destruct (C _ z V).
  + apply H; trivial.
  + destruct H.
    - rewrite (locBlocksTgt_extBlocksTgt _ WD _ H) in E1; congruence.
...
Lemma confined_unch mu (m1 m2 m2':mem) (WD:SM_wd mu) 
      (Fwd: mem_forward m2 m2') (C:confined mu m1 m2 m2'):
      Mem.unchanged_on (extern_out_of_reach mu m1) m2 m2'.
Proof. constructor.
apply mem_forward_nextblock; eassumption.
{ intros b2 z k p [E1 E3(*[E2 E3]*)] V. destruct (C b2 z E1).
  + apply H; trivial.
  + destruct H as [b1 [d [FRG Perm]]].
    destruct (E3 b1 d); [ apply foreign_in_extern; trivial | contradiction |]. 
    apply frgnBlocksSrc_false_foreign_None in H. congruence. }
{ intros b2 z [E1 E3(*[E2 E3]*)] P.
(*  exploit Mem.perm_valid_block; eauto. intros V. *)
...
Definition MSR_confined {C} mu m1 m2 (A2: @MidStepResult C mem) :=
  match A2 with
    MS_Ext _ m2' _ _ => confined mu m1 m2 m2'
  | MS_Ret _ m2' _ => confined mu m1 m2 m2'
  | _ => True
  end.
...
(* Now that we don't require matchstate to hold after each (small) corestep any longer,
  we can require memory injections!*)
; match_inject :
    forall d mu c1 m1 c2 m2 (MC : match_state d mu c1 m1 c2 m2),
           Mem.inject (as_inj mu) m1 m2
